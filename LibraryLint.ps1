...
function Get-LatestRelease {
    ...
    try {
        ...
        # Find the portable ZIP asset (preferred — includes modules and all files)
        $zipAsset = $response.assets | Where-Object { $_.name -match 'Portable\.zip$' } | Select-Object -First 1

        # Fallback to single .ps1 asset if no ZIP available
        $scriptAsset = $response.assets | Where-Object { $_.name -eq "LibraryLint.ps1" } | Select-Object -First 1

        $downloadUrl = if ($zipAsset) {
...
function Install-Update {
    <#
    .SYNOPSIS
        Downloads and installs the latest version (full package or script-only)
    #>
    ...
    if ($latest.IsFullPackage) {
        Write-Host "Update type:     Full package (script + modules)" -ForegroundColor Cyan
    } else {
        Write-Host "Update type:     Script only" -ForegroundColor Cyan
    }
    Write-Host ""
    ...
    try {
        # Get the install directory from the currently running script
        ...
        if ($latest.IsFullPackage) {
            # === Full package update (ZIP with modules, bat, config, etc.) ===

            # Download ZIP to temp
            $tempZip = Join-Path $env:TEMP "LibraryLint_update_$(Get-Random).zip"
            Invoke-WebRequest -Uri $latest.DownloadUrl -OutFile $tempZip -ErrorAction Stop
            Write-Host "  Downloaded package" -ForegroundColor Gray

            # Extract to temp directory
            $tempExtract = Join-Path $env:TEMP "LibraryLint_extract_$(Get-Random)"
            Expand-Archive -Path $tempZip -DestinationPath $tempExtract -Force -ErrorAction Stop
            Remove-Item $tempZip -Force -ErrorAction SilentlyContinue

            # The ZIP contains a subfolder like "LibraryLint-5.4.0" — find it
            $extractedDir = Get-ChildItem -Path $tempExtract -Directory | Select-Object -First 1
            if (-not $extractedDir) {
                # No subfolder — files are at the root of the ZIP
                $sourceDir = $tempExtract
            } else {
                $sourceDir = $extractedDir.FullName
            }

            # Verify the extracted package contains LibraryLint.ps1
            if (-not (Test-Path (Join-Path $sourceDir "LibraryLint.ps1"))) {
                throw "Downloaded package does not contain LibraryLint.ps1"
            }

            # Create backup of entire install directory
            $backupDir = Join-Path $env:TEMP "LibraryLint_backup_$(Get-Date -Format 'yyyyMMdd_HHmmss')"
            Copy-Item -Path $installDir -Destination $backupDir -Recurse -Force
            Write-Host "  Backup created: $backupDir" -ForegroundColor Gray

            # Copy new files over the install directory (preserving user config)
            $updatedFiles = @()
            Get-ChildItem -Path $sourceDir -Recurse -File | ForEach-Object {
                $relativePath = $_.FullName.Substring($sourceDir.Length + 1)
                $destPath = Join-Path $installDir $relativePath

                # Skip user config files (don't overwrite their settings)
                if ($relativePath -match '^config\\config\.json$') { return }
            ...
            Write-Host ""
            Write-Host "Update successful! LibraryLint v$($latest.Version) installed" -ForegroundColor Green
            Write-Host "  Updated $($updatedFiles.Count) files:" -ForegroundColor Gray
            foreach ($f in $updatedFiles) {
                Write-Host "    $f" -ForegroundColor DarkGray
            }
            Write-Host ""
            Write-Host "Please restart LibraryLint to use the new version." -ForegroundColor Yellow
            Write-Host "(Backup at: $backupDir)" -ForegroundColor Gray

        } else {
            # === Legacy single-file update (script only) ===
            ...
            Write-Host ""
            Write-Host "Update successful! LibraryLint v$($latest.Version) installed (script only)" -ForegroundColor Green
            Write-Host ""
            Write-Host "Please restart LibraryLint to use the new version." -ForegroundColor Yellow
        ...
        if ($latest.IsFullPackage -and $backupDir -and (Test-Path $backupDir)) {
...
$script:DependencyCacheFile = Join-Path $script:AppDataFolder "dependency-check-cache.json"
...
$script:DefaultConfig = @{
    ...
    TVDBApiKey = $null
    FanartTVApiKey = $null
    InboxPath = $null  # Single shared inbox for all downloads (movies + TV shows)
    MoviesInboxPath = $null  # Legacy: separate movies inbox (used internally for staging)
    ...
    SFTPWorkingPaths = @()                 # Paths where rTorrent writes in-progress downloads (pre-move). Anything here is incomplete or pending move.
    SFTPWorkingPruneDaysOld = 14           # Min age (days, by newest file mtime) before a working-dir folder is eligible for prune. Protects still-seeding fresh downloads.
...
function Export-Configuration {
    ...
    try {
        ...
        $keyOrder = @(
            ...
            'SFTPRemotePaths', 'SFTPPrunePaths', 'SFTPWorkingPaths', 'SFTPWorkingPruneDaysOld', 'SFTPLocalPath', 'SFTPDeleteAfterDownload',
            'SFTPSpeedLimitKBps', 'SFTPExcludePatterns', 'SFTPUnrarCommand', 'SFTPExtractedSuffix',
            ...
            # Processing - General
            'DryRun', 'GenerateNFO', 'CheckDuplicates', 'EnableUndo',
...
$script:Stats = @{
    ...
    NFOFilesCreated = 0
    NFOFilesRead = 0
    NFOValidationFailures = 0
    NFOValidationDetails = @()
    Errors = 0
    Warnings = 0
    OperationsRetried = 0
    ErrorDetails = @()
...
function Test-MediaInfoInstallation {
    ...
    # Check common install locations (winget, Program Files)
    # Use exact paths first, then wildcard patterns for versioned winget installs
    $exactPaths = @(
        "$env:LOCALAPPDATA\Microsoft\WinGet\Links\MediaInfo.exe",
        "C:\Program Files\MediaInfo\MediaInfo.exe",
        "C:\Program Files (x86)\MediaInfo\MediaInfo.exe"
    )
    foreach ($p in $exactPaths) {
        if (Test-Path $p) {
            $script:Config.MediaInfoPath = $p
            Write-Verbose-Message "MediaInfo found at fallback path: $p"
            return $true
        }
    }

    # Wildcard search for winget versioned install directories
    $wildcardPaths = @(
        "$env:LOCALAPPDATA\Microsoft\WinGet\Packages\MediaArea.MediaInfo*\MediaInfo.exe",
        "$env:LOCALAPPDATA\Microsoft\WinGet\Packages\MediaArea.MediaInfo*\*\MediaInfo.exe"
    )
    foreach ($pattern in $wildcardPaths) {
        $found = Get-Item $pattern -ErrorAction SilentlyContinue | Select-Object -First 1
        if ($found) {
            $script:Config.MediaInfoPath = $found.FullName
            Write-Verbose-Message "MediaInfo found via wildcard: $($found.FullName)"
            return $true
        }
    }
...
function Invoke-EnhancedDuplicateDetection {
    ...
    $bw = 56
    $br = [string]::new([char]0x2550, $bw)
    Write-Host "$([char]0x2554)$br$([char]0x2557)" -ForegroundColor Yellow
    Write-Host "$([char]0x2551)$("         ENHANCED DUPLICATE DETECTION REPORT          ".PadRight($bw))$([char]0x2551)" -ForegroundColor Yellow
    Write-Host "$([char]0x2560)$br$([char]0x2563)" -ForegroundColor Yellow
    Write-Host "$([char]0x2551)$(' ' * $bw)$([char]0x2551)" -ForegroundColor Yellow
    Write-Host "$([char]0x2551)$("  Total duplicate groups: $($duplicates.Count)".PadRight($bw))$([char]0x2551)" -ForegroundColor White
    Write-Host "$([char]0x2551)$("  - High confidence (IMDB/TMDB/Hash): $highConf".PadRight($bw))$([char]0x2551)" -ForegroundColor Green
    Write-Host "$([char]0x2551)$("  - Medium confidence (Title+Year):   $medConf".PadRight($bw))$([char]0x2551)" -ForegroundColor Cyan
    Write-Host "$([char]0x2551)$("  - Low confidence (Title only):      $lowConf".PadRight($bw))$([char]0x2551)" -ForegroundColor DarkYellow
    Write-Host "$([char]0x2551)$(' ' * $bw)$([char]0x2551)" -ForegroundColor Yellow
    Write-Host "$([char]0x2551)$("  Potential space savings: $(Format-FileSize $totalSavings)".PadRight($bw))$([char]0x2551)" -ForegroundColor Magenta
    Write-Host "$([char]0x2551)$(' ' * $bw)$([char]0x2551)" -ForegroundColor Yellow
    Write-Host "$([char]0x255A)$br$([char]0x255D)" -ForegroundColor Yellow
    ...
    switch ($action) {
        ...
        "4" {
            # Export to HTML
            $htmlPath = Join-Path $script:ReportsFolder "DuplicateReport_$(Get-Date -Format 'yyyyMMdd_HHmmss').html"
            Export-DuplicateReport -Duplicates $duplicates -OutputPath $htmlPath
            Write-Host "`nReport exported to: $htmlPath" -ForegroundColor Green
        }
        "5" {
...
<#
.SYNOPSIS
    Exports duplicate report to HTML
.PARAMETER Duplicates
...
function Export-DuplicateReport {
    ...
    foreach ($group in $Duplicates) {
        ...
        foreach ($movie in $sorted) {
            ...
            $actionClass = if ($first) { "keep" } else { "delete" }
            $confidence = $movie.Confidence
            $confClass = switch ($confidence) { "High" { "high" } "Medium" { "medium" } default { "low" } }
            $size = Format-FileSize $movie.FileSize

            $tableRows += @"
            <tr class="$actionClass">
                <td>$groupNum</td>
                <td class="$confClass">$confidence</td>
                <td>$(($movie.MatchType -join ', '))</td>
                <td><strong>$action</strong></td>
                <td>$($movie.OriginalName)</td>
                <td>$($movie.Quality.Score)</td>
                <td>$($movie.Quality.Resolution)</td>
                <td>$size</td>
            </tr>
"@
            $first = $false
...
<head>
    <meta charset="UTF-8">
    <title>LibraryLint Duplicate Report</title>
    <style>
        body { font-family: 'Segoe UI', sans-serif; background: #1a1a2e; color: #eee; padding: 20px; }
        h1 { color: #00d4ff; text-align: center; }
        p.meta { text-align: center; color: #888; margin-bottom: 20px; }
        ...
        input.search { width: 100%; padding: 10px; margin-bottom: 15px; background: #16213e; border: 2px solid #0f3460; border-radius: 6px; color: #eee; font-size: 1em; }
        input.search:focus { outline: none; border-color: #00d4ff; }
        ...
        tr.keep td:nth-child(4) { color: #00ff88; }
        tr.delete td:nth-child(4) { color: #ff4444; }
        .high { color: #00ff88; }
        .medium { color: #ffaa00; }
        .low { color: #ff6666; }
...
<body>
    ...
    <p class="meta">Generated: $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') | Groups: $($Duplicates.Count)</p>
    <input type="text" class="search" placeholder="Search..." onkeyup="filterTable(this.value)">
    <table id="tbl">
        <thead><tr><th>Group</th><th>Confidence</th><th>Match Type</th><th>Action</th><th>Folder</th><th>Score</th><th>Resolution</th><th>Size</th></tr></thead>
        <tbody>$tableRows</tbody>
    </table>
    <div class="footer">Generated by LibraryLint v$($script:AppVersion)</div>
    <script>
...
function Invoke-ExportMenu {
    ...
    Write-Host "1. Export to HTML (visual report, opens in browser)"
    Write-Host "2. Export to JSON (backup/integration)"
    Write-Host "3. Export All Formats"
    Write-Host "0. Cancel"

    $choice = Read-Host "`nSelect export format"

    switch ($choice) {
        "1" { Export-LibraryToHTML -Path $Path -MediaType $MediaType }
        "2" { Export-LibraryToJSON -Path $Path -MediaType $MediaType }
        "3" {
    ...
    while ($true) {
        ...
        switch ($choice) {
            ...
            "1" {
                ...
                Write-Host "  1. Download new movies/shows to your inbox folder"
                Write-Host "  2. Run 'Process Inbox' - content is auto-detected as Movies or TV"
...
function Install-WingetPackage {
    ...
    try {
        $process = Start-Process -FilePath "winget" -ArgumentList "install", "--id", $PackageId, "--exact", "--accept-source-agreements", "--accept-package-agreements", "-h" -Wait -PassThru -NoNewWindow
        # Exit code 0 = success, -1978335189 (0x8A150013) = already installed
...
function Install-MissingDependencies {
    ...
    if (-not (Test-WingetAvailable)) {
        ...
        foreach ($dep in $missing) {
            ...
            if ($urls.ContainsKey($dep.Key)) {
                Write-Host "  $($dep.Key): $($urls[$dep.Key])" -ForegroundColor Cyan
            }
        }
        return $Dependencies
    }
    ...
    $installMap = @{
        ...
        "MediaInfo" = @{ PackageId = "MediaArea.MediaInfo"; PostInstall = {
            ...
            # If still not found, refresh PATH and retry
            if (-not $script:Config.MediaInfoPath -or -not (Test-Path $script:Config.MediaInfoPath)) {
                $env:Path = [System.Environment]::GetEnvironmentVariable("Path", "Machine") + ";" + [System.Environment]::GetEnvironmentVariable("Path", "User")
                $null = Test-MediaInfoInstallation
            }
        }; ManualUrl = "https://mediaarea.net/en/MediaInfo/Download/Windows" }
    ...
    foreach ($dep in $missing) {
        ...
        if ($installMap.ContainsKey($depName)) {
            ...
            } elseif ($info.ManualUrl) {
                Write-Host "  You can install $depName manually from:" -ForegroundColor Yellow
                Write-Host "  $($info.ManualUrl)" -ForegroundColor Cyan
            }
        }
    }
...
function Test-DependencyUpdates {
    param(
        # When -Brief is set, the "Update all via winget?" prompt is skipped.
        # Used by background/automated callers. Default launch-time behavior
        # DOES prompt so the user can decline (network, mid-task, regression
        # caution, etc.).
        [switch]$Brief,

        # When > 0, use the cache file if its age is below this threshold —
        # avoids re-running winget upgrade (5-15s) on every launch. The
        # menu-driven flow passes 0 (no caching) to force a fresh check.
    ...
    # Try cache before invoking winget. Stored as a small JSON next to other
    # AppData artifacts. Cleared after a successful install so the next
    # launch reflects post-install reality.
    $usedCache = $false
    $updatesAvailable = @()
    if ($MaxCacheAgeHours -gt 0 -and (Test-Path -LiteralPath $script:DependencyCacheFile)) {
        try {
            $cache = Get-Content -LiteralPath $script:DependencyCacheFile -Raw | ConvertFrom-Json
            if ($cache.LastCheck) {
                $cacheAge = (Get-Date) - [DateTime]::Parse($cache.LastCheck)
                if ($cacheAge.TotalHours -lt $MaxCacheAgeHours) {
                    $usedCache = $true
                    $updatesAvailable = @($cache.UpdatesAvailable | ForEach-Object {
                        @{ Name = $_.Name; PackageId = $_.PackageId; Detail = $_.Detail }
                    })
                    $hoursAgo = [math]::Round($cacheAge.TotalHours, 1)
                    Write-Host "Using cached result from ${hoursAgo}h ago." -ForegroundColor DarkGray
                }
            }
        } catch {
            # Cache unreadable — fall through to fresh check
        }
    }
    ...
    if (-not $usedCache) {
        ...
        foreach ($dep in $depPackages.GetEnumerator()) {
            if ($upgradeText -match [regex]::Escape($dep.Value)) {
                ...
                $detailText = if ($matchLine) { $matchLine.ToString().Trim() } else { $null }
                $updatesAvailable += @{ Name = $dep.Key; PackageId = $dep.Value; Detail = $detailText }
            }
        }

        # Save the fresh result for next launch
        try {
            $cacheObj = @{
                LastCheck = (Get-Date).ToString("o")
                UpdatesAvailable = $updatesAvailable
            }
            $cacheObj | ConvertTo-Json -Depth 4 | Out-File -LiteralPath $script:DependencyCacheFile -Encoding UTF8 -Force
        } catch {
            # Cache write failure is non-fatal — just means next launch hits winget again
        }
    }

    # Display status — single block for both cached and fresh paths
    foreach ($dep in $depPackages.GetEnumerator()) {
        $update = $updatesAvailable | Where-Object { $_.PackageId -eq $dep.Value } | Select-Object -First 1
        if ($update) {
            Write-Host "  $($dep.Key): Update available" -ForegroundColor Yellow
            if ($update.Detail) {
                Write-Host "    $($update.Detail)" -ForegroundColor DarkGray
            }
        } else {
    ...
    # yt-dlp self-update — only run on fresh checks since the cached result
    # doesn't reflect whether yt-dlp's own self-update has anything to do.
    if (-not $usedCache -and (Test-YtDlpInstallation)) {
        Update-YtDlp
    }
    ...
    # Brief mode — inform but don't prompt
    if ($Brief) {
        Write-Host ""
        Write-Host "  Run Utilities > Check for Dependency Updates to install." -ForegroundColor DarkGray
        return
    }
    ...
    # Invalidate cache — the upgrade list is now stale (the things we just
    # installed are no longer pending). Next launch's check will rebuild it.
    if (Test-Path -LiteralPath $script:DependencyCacheFile) {
        Remove-Item -LiteralPath $script:DependencyCacheFile -Force -ErrorAction SilentlyContinue
    }
    ...
    try {
        ...
        foreach ($subtitle in $subtitleFiles) {
            ...
            if ($script:Config.KeepSubtitles -and $isPreferred) {
                # If subtitle is in a subfolder (e.g., Subs/), move it next to the video
                # and rename to match the video basename for player auto-detection
                $movieFolder = $subtitle.DirectoryName
                $isInSubfolder = $false

                # Walk up to find the movie folder that contains a video file
                $testFolder = $subtitle.Directory
                while ($testFolder -and $testFolder.FullName -ne $Path) {
                    $parentVideos = Get-ChildItem -LiteralPath $testFolder.Parent.FullName -File -ErrorAction SilentlyContinue |
                        Where-Object { $script:Config.VideoExtensions -contains $_.Extension.ToLower() -and $_.Name -notmatch '-trailer\.' }
                    if ($parentVideos) {
                        $movieFolder = $testFolder.Parent.FullName
                        $isInSubfolder = $true
                        break
                    }
                ...
                if ($isInSubfolder -and -not $script:Config.DryRun) {
                    # Find the main video file to match against
                    ...
                    if ($videoFile) {
                        ...
                        # Detect language suffix from the subtitle name
                        $detectedLang = ""
                        if ($subtitleNameLower -match '[\._](eng|en|english)') { $detectedLang = ".en" }
                        elseif ($subtitleNameLower -match '[\._](spa|es|spanish)') { $detectedLang = ".es" }
                        elseif ($subtitleNameLower -match '[\._](fre|fr|french)') { $detectedLang = ".fr" }
                        elseif ($subtitleNameLower -match '[\._](ger|de|german)') { $detectedLang = ".de" }
                        elseif ($subtitleNameLower -match '[\._](ita|it|italian)') { $detectedLang = ".it" }
                        elseif ($subtitleNameLower -match '[\._](por|pt|portuguese)') { $detectedLang = ".pt" }
                        elseif ($subtitleNameLower -match '[\._](rus|ru|russian)') { $detectedLang = ".ru" }
                        elseif ($subtitleNameLower -match '[\._](jpn|ja|japanese)') { $detectedLang = ".ja" }
                        elseif ($subtitleNameLower -match '[\._](chi|zh|chinese)') { $detectedLang = ".zh" }
                        elseif ($subtitleNameLower -match '[\._](kor|ko|korean)') { $detectedLang = ".ko" }
                        elseif ($subtitleNameLower -match '[\._](ara|ar|arabic)') { $detectedLang = ".ar" }
                        elseif ($subtitleNameLower -match '[\._](hin|hi|hindi)') { $detectedLang = ".hi" }
                        elseif ($subtitleNameLower -match '[\._](dut|nl|dutch)') { $detectedLang = ".nl" }
                        elseif ($subtitleNameLower -match '[\._](pol|pl|polish)') { $detectedLang = ".pl" }
                        elseif ($subtitleNameLower -match '[\._](swe|sv|swedish)') { $detectedLang = ".sv" }
                        elseif ($subtitleNameLower -match '[\._](nor|no|norwegian)') { $detectedLang = ".no" }
                        elseif ($subtitleNameLower -match '[\._](dan|da|danish)') { $detectedLang = ".da" }
                        elseif ($subtitleNameLower -match '[\._](fin|fi|finnish)') { $detectedLang = ".fi" }

                        $newSubName = "$videoBaseName$detectedLang$($subtitle.Extension)"
                        $newSubPath = Join-Path $movieFolder $newSubName

                        # Avoid overwriting an existing subtitle with the same name
                        if (-not (Test-Path -LiteralPath $newSubPath)) {
                            try {
                                Move-Item -LiteralPath $subtitle.FullName -Destination $newSubPath -Force
                                Write-Host "Moved subtitle: $($subtitle.Name) -> $newSubName" -ForegroundColor Green
                                Write-Log "Moved subtitle to video folder: $($subtitle.FullName) -> $newSubPath" "INFO"
                            }
                            catch {
                                Write-Host "Warning: Could not move subtitle $($subtitle.Name): $_" -ForegroundColor Yellow
                                Write-Log "Error moving subtitle: $_" "ERROR"
                            }
                        } else {
                            Write-Host "Subtitle already exists, skipping: $newSubName" -ForegroundColor Gray
                            Write-Log "Subtitle already exists at destination, skipping: $newSubPath" "DEBUG"
                        }
                    } else {
                        Write-Host "Keeping subtitle (no video found): $($subtitle.Name)" -ForegroundColor Green
                        Write-Log "Keeping subtitle (no video in parent): $($subtitle.FullName)" "INFO"
                    }
                } elseif ($script:Config.DryRun -and $isInSubfolder) {
                    Write-Host "[DRY-RUN] Would move subtitle to video folder: $($subtitle.Name)" -ForegroundColor Yellow
                    Write-Log "Would move subtitle to video folder: $($subtitle.FullName)" "DRY-RUN"
                } else {
...
<#
.SYNOPSIS
    Validates that an NFO file's metadata matches the folder it's in
.PARAMETER NfoPath
    Path to the NFO file
.PARAMETER FolderName
    The folder name to validate against
.OUTPUTS
    Hashtable with Match (bool), Confidence, Details, NFOTitle, NFOYear, FolderTitle, FolderYear
#>
function Test-NFOMatchesFolder {
    param(
        [string]$NfoPath,
        [string]$FolderName
    )
    ...
    # Read the NFO
    $nfoMetadata = Read-NFOFile -NfoPath $NfoPath
    if (-not $nfoMetadata -or -not $nfoMetadata.Title) {
        $result.Details = "NFO unreadable or has no title"
        return $result
    }

    $result.NFOTitle = $nfoMetadata.Title
    $result.NFOYear = $nfoMetadata.Year

    # Extract title and year from folder name
    $folderTitle = $FolderName
    $folderYear = $null
    if ($FolderName -match '^(.+?)\s*\((\d{4})\)\s*$') {
        $folderTitle = $Matches[1].Trim()
        $folderYear = $Matches[2]
    } elseif ($FolderName -match '^(.+?)\s+((?:19|20)\d{2})\s*$') {
        $folderTitle = $Matches[1].Trim()
        $folderYear = $Matches[2]
    }

    $result.FolderTitle = $folderTitle
    $result.FolderYear = $folderYear
    ...
    $yearOK = $true
    if ($folderYear -and $nfoMetadata.Year) {
    ...
    if (-not $titleMatch -and $normFolder -and $normNFO) {
        if ($normFolder.Contains($normNFO) -or $normNFO.Contains($normFolder)) {
            $titleMatch = $true
            $confidence = "Medium"
        }
        if (-not $titleMatch -and $normOriginal) {
            if ($normFolder.Contains($normOriginal) -or $normOriginal.Contains($normFolder)) {
                $titleMatch = $true
                $confidence = "Medium"
            }
        }
    ...
    if (-not $titleMatch -and $normFolder -and $normNFO) {
        $folderTokens = @($normFolder -split '\s+')
        $nfoTokens = @($normNFO -split '\s+')
        if ($folderTokens.Count -gt 0 -and $nfoTokens.Count -gt 0) {
            $intersection = @($folderTokens | Where-Object { $nfoTokens -contains $_ }).Count
            $union = ($folderTokens + $nfoTokens | Select-Object -Unique).Count
            if ($union -gt 0) {
    ...
    if ($titleMatch) {
        ...
        $result.Details = "Matched ($confidence confidence)"
    } else {
        $result.Confidence = "Mismatch"
        $result.Details = "Title mismatch: folder='$folderTitle' NFO='$($nfoMetadata.Title)'"
    }
...
function Restore-FromQuarantine {
    ...
    foreach ($folder in $folders) {
        ...
        if (Test-Path -LiteralPath $destination) {
            # Largest non-trailer/non-sample video = the main feature. Picking
            # the alphabetically-first file lets a sample/trailer's quality
            # score gate the import of the real movie.
            $pickMainVideo = {
                param($Path)
                Get-ChildItem -LiteralPath $Path -File -ErrorAction SilentlyContinue |
                    Where-Object {
                        $_.Extension -match '\.(mkv|mp4|avi|mov|wmv|m4v)$' -and
                        $_.BaseName -notmatch '(?i)(-trailer|\.trailer|-sample|\.sample|-featurette|behindthescenes|extras?)$'
                    } |
                    Sort-Object Length -Descending |
                    Select-Object -First 1
            }
            $quarantinedVideo = & $pickMainVideo $folder.FullName
            $libraryVideo     = & $pickMainVideo $destination
        ...
        if ($script:Config.TMDBApiKey) {
            ...
            if ($searchResult -and $searchResult.Id) {
                ...
                if ($tmdbMetadata) {
                    ...
                    # Validate NFO matches folder name
                    $validation = Test-NFOMatchesFolder -NfoPath $nfoPath -FolderName $videoFile.Directory.Name
                    if (-not $validation.Match) {
                        $mismatchMsg = "NFO mismatch in '$($videoFile.Directory.Name)': NFO='$($validation.NFOTitle) ($($validation.NFOYear))' - $($validation.Details)"
                        Write-Host "  NFO MISMATCH: TMDB returned wrong movie" -ForegroundColor Red
                        Write-Host "    Folder: $($videoFile.Directory.Name)" -ForegroundColor Yellow
                        Write-Host "    NFO:    $($validation.NFOTitle) ($($validation.NFOYear))" -ForegroundColor Yellow
                        Write-Log $mismatchMsg "ERROR"
                        $script:Stats.NFOValidationFailures++
                        $script:Stats.NFOValidationDetails += @{
                            FolderName = $videoFile.Directory.Name
                            FolderPath = $videoFile.DirectoryName
                    ...
                    } elseif ($validation.Confidence -eq "Low") {
                        Write-Host "  Note: NFO matched with low confidence ($($validation.Details))" -ForegroundColor Yellow
...
<#
.SYNOPSIS
    Renames actor image files in .actors/ folders to match Kodi's
    underscore-based lookup convention.
.DESCRIPTION
    Kodi looks up actor images by replacing spaces in the NFO actor name
    with underscores: "Brad Pitt" -> ".actors/Brad_Pitt.jpg". The original
    artwork downloader saved files with spaces preserved ("Brad Pitt.jpg"),
    which Kodi never finds — causing the "no actor thumbnails despite
    .actors folder existing" symptom.

    This walks every .actors folder under the given root paths and renames
    any file whose basename contains whitespace. Skips files that already
    have an underscore-version present (so re-runs are safe and don't
    overwrite). Idempotent — files already named correctly are ignored.

    The download path itself was fixed in the same release; this function
    is the one-shot migration for libraries populated under the old code.
.PARAMETER Path
    One or more library root paths (e.g. MoviesLibraryPath, TVShowsLibraryPath).
.PARAMETER WhatIf
    If specified, shows what would be renamed without making changes.
#>
function Repair-ActorImageNames {
    param(
        [Parameter(Mandatory)]
        [string[]]$Path,
        [switch]$WhatIf
    ...
    Write-Host "`n--- Fix Actor Image Names ---" -ForegroundColor Yellow
    if ($WhatIf) {
    ...
    foreach ($root in $Path) {
        ...
        if (-not (Test-Path -LiteralPath $root)) {
            Write-Host "  Skipping: $root (not found)" -ForegroundColor Gray
            continue
        }

        Write-Host "  Scanning $root..." -ForegroundColor Gray
        # -Force needed because .actors is a dot-prefix (hidden) folder.
        # -Filter handles the literal name match; no glob magic at play.
        $actorsFolders = Get-ChildItem -LiteralPath $root -Directory -Recurse -Force -Filter '.actors' -ErrorAction SilentlyContinue

        foreach ($af in $actorsFolders) {
            $stats.FoldersScanned++
            $files = Get-ChildItem -LiteralPath $af.FullName -File -ErrorAction SilentlyContinue
            foreach ($file in $files) {
                # Only rename files whose basename contains whitespace —
                # underscore-already files are correctly named.
                if ($file.BaseName -notmatch '\s') { continue }

                $newName = ($file.BaseName -replace '\s+', '_') + $file.Extension
                $newPath = Join-Path $af.FullName $newName

                if (Test-Path -LiteralPath $newPath) {
                    # An underscore-version already exists alongside the
                    # space-version (likely from a re-run after the fix).
                    # Don't overwrite — leave the user to resolve.
                    Write-Host "  [skip]         $($af.Parent.Name)\.actors\$($file.Name) — $newName already exists" -ForegroundColor DarkGray
                    $stats.FilesSkipped++
                    continue
                }

                $action = if ($WhatIf) { '[would rename]' } else { '[renamed] ' }
                Write-Host "  $action $($af.Parent.Name)\.actors\$($file.Name) -> $newName" -ForegroundColor Gray
    ...
    Write-Host ""
    Write-Host ("  .actors folders scanned:   {0}" -f $stats.FoldersScanned) -ForegroundColor White
    $verb = if ($WhatIf) { 'would rename:             ' } else { 'renamed:                  ' }
    Write-Host ("  $verb {0}" -f $stats.FilesRenamed) -ForegroundColor Green
    Write-Host ("  skipped (already exists): {0}" -f $stats.FilesSkipped) -ForegroundColor Gray
...
function Repair-MovieFolderYears {
    ...
    foreach ($folder in $folders) {
        ...
        # Extract existing year if present (with or without parentheses)
        $existingYear = $null
        if ($folder.Name -match '\(((19|20)\d{2})\)') {
            $existingYear = $matches[1]
        } elseif ($folder.Name -match '(?:^|[\s\.\-_])((19|20)\d{2})(?:[\s\.\-_]|$)') {
            $existingYear = $matches[1]
        }

        # Check if folder name matches NFO title (catches dirty titles and wrong renames)
        ...
        if ($nfoFile) {
            ...
            if ($nfoMetadataEarly -and $nfoMetadataEarly.Title) {
                # Extract title from folder name (without parenthesized year)
                $folderTitle = ($folder.Name -replace '\s*\(\d{4}\)\s*$', '').Trim()
                if ($folderTitle -and $nfoMetadataEarly.Title -and
                    ($folderTitle -ine $nfoMetadataEarly.Title.Trim())) {
                    if ($hasYear) {
                        # Folder already has proper Title (Year) format — only flag mismatch if
                        # the NFO title is a shorter/cleaner version of the folder title (folder has extra tags).
                        # If the NFO title is longer or completely different, the NFO is likely wrong.
                        $nfoTitleClean = $nfoMetadataEarly.Title.Trim().ToLower() -replace '[^a-z0-9\s]', '' -replace '\s+', ' '
                        $folderTitleClean = $folderTitle.ToLower() -replace '[^a-z0-9\s]', '' -replace '\s+', ' '
                        if ($nfoTitleClean -and $folderTitleClean -and
                            $folderTitleClean.Contains($nfoTitleClean) -and
                            $folderTitleClean.Length -gt $nfoTitleClean.Length) {
                            # Folder title contains the NFO title plus extra content — folder has tags/junk
                            $nfoMismatch = $true
                        } else {
                            Write-Log "NFO mismatch in '$($folder.Name)' - NFO='$($nfoMetadataEarly.Title)' looks wrong; keeping folder name" "DEBUG"
                        }
                    } else {
                        $nfoMismatch = $true
                    }
        ...
        # Use -Strict to preserve leading articles (The, A, An) — needed for TMDB search
        $titleInfo = Get-NormalizedTitle -Name $folder.Name -Strict
        ...
        if ($script:Config.TMDBApiKey) {
            ...
            # If search with year fails, retry without year filter
            if (-not $tmdbResult -and $searchYear) {
                $tmdbResult = Search-TMDBMovie -Title $cleanTitle -ApiKey $script:Config.TMDBApiKey
            }
            if ($tmdbResult) {
        ...
        if ($finalYear -or $needsCasing -or $needsTitleFix) {
            ...
            # Priority for title: TMDB > NFO > folder name (cleaned)
            # TMDB and NFO titles are authoritative — don't post-process them
            # Only the cleanTitle fallback needs tag/year stripping (already done by Get-NormalizedTitle)
            $displayTitle = if ($tmdbResult -and $tmdbResult.Title) {
                $tmdbResult.Title
            } elseif ($nfoMetadata -and $nfoMetadata.Title) {
                $nfoMetadata.Title
            } else {
                # cleanTitle from Get-NormalizedTitle already has tags and years stripped; apply title casing
...
function Repair-TVShowFolderNames {
    ...
    foreach ($folder in $folders) {
        ...
        if ($finalYear -or $needsCasing -or $needsTitleFix) {
            ...
            # Priority for title: TVDB > NFO > folder name (cleaned)
            # TVDB and NFO titles are authoritative — don't post-process them
            $displayTitle = if ($tvdbResult -and $tvdbResult.Title) {
                $tvdbResult.Title
            } elseif ($nfoMetadata -and $nfoMetadata.Title) {
                $nfoMetadata.Title
            } else {
                # cleanTitle already has tags and years stripped; apply title casing
                (Get-Culture).TextInfo.ToTitleCase($cleanTitle.ToLower())
            }
...
function Move-MoviesToLibrary {
    ...
    if ($holdBack.Count -gt 0) {
        # Move held-back movies to a _Review sibling folder so the inbox stays
        # clean for the next routine run. _Review is the same quarantine the
        # NFO-mismatch resolver uses, keeping all "needs human attention"
        # folders in one place.
        $reviewPath = Join-Path (Split-Path $InboxPath -Parent) "_Review"
        ...
        foreach ($item in $holdBack) {
            $folder = $item.Folder
            $destPath = Join-Path $reviewPath $folder.Name
            $counter = 1
            while (Test-Path -LiteralPath $destPath) {
                $counter++
                $destPath = Join-Path $reviewPath "$($folder.Name) ($counter)"
            }
            try {
                Move-Item -LiteralPath $folder.FullName -Destination $destPath -Force -ErrorAction Stop
                Write-Host "    - $($folder.Name) ($($item.Reason)) -> _Review" -ForegroundColor Yellow
                Write-Log "Held back, moved to _Review: '$($folder.Name)' ($($item.Reason))" "INFO"
                $movedCount++
            } catch {
                Write-Host "    - $($folder.Name) ($($item.Reason)) — MOVE FAILED: $_" -ForegroundColor Red
                Write-Log "Failed to move held-back '$($folder.Name)' to _Review: $_" "ERROR"
                $moveFailures += $folder.Name
            }
        }
        Write-Host ""
        Write-Host "  $movedCount moved to: $reviewPath" -ForegroundColor DarkGray
        if ($moveFailures.Count -gt 0) {
            Write-Host "  $($moveFailures.Count) failed to move and remain in inbox." -ForegroundColor Red
        }
        Write-Host "  Fix metadata in _Review, then move folders back to inbox to retry." -ForegroundColor DarkGray
        Write-Host ""
    }
    ...
    foreach ($folder in $transferReady) {
        ...
        if (Test-Path -LiteralPath $destPath) {
            # Pick the main feature, not whatever sorts alphabetically first.
            # Trailers/samples/extras can sort ahead of the feature depending on
            # how the release names them (e.g. "Title-trailer.mkv" vs
            # "Title.mkv" → '-' < '.' so trailer wins), which would let a
            # 50 MB clip's quality score gate the import of the real movie.
            $pickMainVideo = {
                param($Path)
                Get-ChildItem -LiteralPath $Path -File -ErrorAction SilentlyContinue |
                    Where-Object {
            ...
            $inboxVideo   = & $pickMainVideo $folder.FullName
            $libraryVideo = & $pickMainVideo $destPath
            ...
            if ($inboxVideo -and $libraryVideo) {
                ...
                # Skipped — surface the comparison so the user can audit. Without
                # this, a wrong decision is invisible; previously the diagnostic
                # only printed on the upgrade branch.
                Write-Host " [EXISTS - skipped]" -ForegroundColor Yellow
                Write-Host "      Library: $($libraryQuality.Resolution) $($libraryQuality.Codec) (Score: $($libraryQuality.Score), $(Format-FileSize $librarySize))" -ForegroundColor DarkGray
                Write-Host "      Inbox:   $($inboxQuality.Resolution) $($inboxQuality.Codec) (Score: $($inboxQuality.Score), $(Format-FileSize $inboxSize))" -ForegroundColor DarkGray
            } else {
                # Couldn't score one side (no eligible video) — fall back to
                # name-collision skip with no quality detail.
                Write-Host " [EXISTS - skipped]" -ForegroundColor Yellow
            }
    ...
    if ($Metadata.Actors -and $Metadata.Actors.Count -gt 0) {
        ...
        if ($actorsWithThumbs.Count -gt 0) {
            ...
            foreach ($actor in $actorsWithThumbs) {
                # Sanitize actor name for filename. Spaces -> underscores
                # because Kodi looks up actor images by NFO actor name with
                # spaces replaced by underscores ("Brad Pitt" -> "Brad_Pitt.jpg").
                # The space-preserved variant ("Brad Pitt.jpg") sits in .actors
                # but Kodi never finds it.
                $safeActorName = $actor.Name -replace '[\\/:*?"<>|\s]', '_'
...
<#
.SYNOPSIS
    Cache file path for the per-folder TMDB collection-lookup cache.
.DESCRIPTION
    Matches the SFTP tracking-file convention: a JSON file under
    `%LOCALAPPDATA%\LibraryLint\`. The cache keys folder name → its last-known
    TMDB ID, collection membership, NFO mtime, and check timestamp so the
    next run can skip both the NFO read and the TMDB API call when nothing
    has changed.
#>
function Get-MovieCollectionCachePath {
    return "$env:LOCALAPPDATA\LibraryLint\movie_collection_cache.json"
}

function Read-MovieCollectionCache {
    $path = Get-MovieCollectionCachePath
    ...
    try {
        ...
        # The next save will overwrite.
        Write-Host "  Cache file unreadable, starting fresh: $_" -ForegroundColor DarkYellow
        return @{ version = 1; movies = @{} }
    }
}

function Save-MovieCollectionCache {
    param([hashtable]$Cache)
    $path = Get-MovieCollectionCachePath
    $Cache | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath $path -Encoding UTF8
}
...
.DESCRIPTION
    ...
    Maintains a local cache (movie_collection_cache.json) keyed by folder
    name. Cache entries store TMDB ID, collection membership, NFO mtime,
    and a CheckedAt timestamp. A cache hit (NFO mtime unchanged AND entry
    fresher than $maxCacheAgeDays) skips both the NFO read and the TMDB
    API call for that folder.
.PARAMETER MoviesPath
    Path to the movies library
.PARAMETER SetArtworkPath
    Path to the Movie Set Artwork root folder (MSIF)
.PARAMETER ForceRefresh
    Bypass the cache and re-query everything. Use when TMDB has updated a
    collection's members and you don't want to wait for the TTL.
#>
function Invoke-MovieSetArtworkDownload {
    ...
    if ($ForceRefresh) {
        Write-Host "  Cache:        bypassed (-ForceRefresh)" -ForegroundColor Yellow
    } else {
        Write-Host "  Cache:        $(Get-MovieCollectionCachePath)" -ForegroundColor DarkGray
    }
    Write-Host ""

    # Cache: per-folder TMDB+collection lookup, keyed by folder name. NFO mtime
    # is the trust anchor — same mtime + entry younger than the TTL means the
    # TMDB membership couldn't have meaningfully changed, so we skip the API
    # call entirely.
    $cache = Read-MovieCollectionCache
    $maxCacheAgeDays = 90
    $cacheCutoff = (Get-Date).ToUniversalTime().AddDays(-$maxCacheAgeDays)
    ...
    foreach ($folder in $folders) {
        $scannedCount++
        $seenFolders[$folder.Name] = $true

        # Locate the canonical NFO once — used both for cache validation and
        # by the original NFO-gate path below. Matches Test-FolderHasValidNFO's
        # selection rules.
        $nfoFile = Get-ChildItem -LiteralPath $folder.FullName -Filter "*.nfo" -File -ErrorAction SilentlyContinue |
            Where-Object { $_.Name -ne 'tvshow.nfo' -and $_.Name -notmatch '-trailer\.nfo$' } |
            Select-Object -First 1
        $nfoMtime = if ($nfoFile) { $nfoFile.LastWriteTimeUtc.ToString('o') } else { $null }

        # Cache hit?
        if (-not $ForceRefresh -and $nfoFile -and $cache.movies.ContainsKey($folder.Name)) {
            $cached = $cache.movies[$folder.Name]
            $checkedAt = $null
            if ($cached.CheckedAt) {
                try { $checkedAt = ([DateTime]::Parse($cached.CheckedAt)).ToUniversalTime() } catch {}
            }
            $isFresh = $checkedAt -and $checkedAt -gt $cacheCutoff
            $mtimeMatches = ($cached.NFOMtime -and $cached.NFOMtime -eq $nfoMtime)

            if ($isFresh -and $mtimeMatches) {
                $cacheHits++
                switch ($cached.Status) {
                    'BadNFO'          { $skippedBadNfo++ }
                    'NoTMDB'          { $noTmdbIdCount++ }
                    'NotInCollection' { $notInCollectionCount++ }
                    'InCollection'    {
                        $colId = [int]$cached.CollectionID
                ...
                if ($scannedCount % 25 -eq 0) {
                    Write-Host "  Scanned $scannedCount / $totalFolders movies (cache: $cacheHits hit / $cacheMisses miss)..." -ForegroundColor Gray
                }
                continue
            }
        }

        # Cache miss — run the full lookup, write the result back.
        $cacheMisses++
        $entry = @{
            NFOMtime  = $nfoMtime
        ...
        if (-not $gate.Valid) {
            ...
            $cache.movies[$folder.Name] = $entry
            continue
        }
        ...
        if (-not $tmdbId) {
            $noTmdbIdCount++
            $entry.Status = 'NoTMDB'
            if ($gate.Metadata.Title) { $entry.Title = $gate.Metadata.Title }
            $cache.movies[$folder.Name] = $entry
            continue
        }
        $entry.TMDBID = $tmdbId
        if ($gate.Metadata.Title) { $entry.Title = $gate.Metadata.Title }
        if ($gate.Metadata.Year)  { $entry.Year  = $gate.Metadata.Year }
        ...
        if ($metadata -and $metadata.CollectionId) {
            ...
            $collections[$colId].Movies += $folder.Name
            $entry.Status         = 'InCollection'
            $entry.CollectionID   = $colId
            $entry.CollectionName = $metadata.CollectionName
        } else {
            $notInCollectionCount++
            $entry.Status = 'NotInCollection'
        }
        $cache.movies[$folder.Name] = $entry

        # Rate limiting — only when we actually hit the API.
    ...
    # Prune cache entries for folders no longer in the library (renamed, deleted).
    $prunedCount = 0
    foreach ($name in @($cache.movies.Keys)) {
        if (-not $seenFolders.ContainsKey($name)) {
            $cache.movies.Remove($name) | Out-Null
            $prunedCount++
        }
    }
    Save-MovieCollectionCache -Cache $cache

    # Phase 1 summary
    Write-Host "`n--- Scan Complete ---" -ForegroundColor Green
    Write-Host "  Movies scanned:        $scannedCount" -ForegroundColor White
    Write-Host "  Cache hits:            $cacheHits" -ForegroundColor Gray
    Write-Host "  Cache misses (queried):$cacheMisses" -ForegroundColor Gray
    if ($prunedCount -gt 0) {
        Write-Host "  Cache pruned (gone):   $prunedCount" -ForegroundColor DarkGray
    }
...
function Invoke-ArtworkManagement {
    ...
    foreach ($folder in $folders) {
                ...
                if ($WhatIf) {
                    ...
                    try {
                        if ($isFolder) {
                            ...
                            if (-not $Quiet) { Write-Host "  Removed folder: $itemName" -ForegroundColor Gray }
                            $foldersRemoved++
                        } else {
                            Remove-Item -LiteralPath $item -Force -ErrorAction Stop
                            if (-not $Quiet) { Write-Host "  Removed: $itemName" -ForegroundColor Gray }
                            $filesRemoved++
                        }
                    } catch {
        ...
        # Quiet mode: show a single progress line
        if ($Quiet) {
            $pct = [math]::Round(($processedCount / $totalFolders) * 100)
            Write-Host "`r  Clearing artwork... $processedCount/$totalFolders folders ($($filesRemoved + $foldersRemoved) files removed)   " -NoNewline -ForegroundColor DarkGray
        }
    }

    if ($Quiet) {
        Write-Host "`r  Cleared $($filesRemoved + $foldersRemoved) files from $processedCount folders                              " -ForegroundColor Gray
    } else {
...
function Save-MovieTrailer {
    ...
    # Filter out invalid YouTube video IDs (valid IDs are exactly 11 characters)
    $keysToTry = @($keysToTry | Where-Object { $_ -and $_.Length -ge 10 })
    if ($keysToTry.Count -eq 0) {
        Write-Log "No valid trailer keys for $MovieTitle (all keys too short)" "DEBUG"
        return $false
    }
    ...
    foreach ($key in $keysToTry) {
        ...
        try {
            ...
            # Build argument string (single string avoids Start-Process array quoting issues
            # that truncate URLs at the ? character)
            $ytdlpArgs = "--no-playlist -f `"$formatSpec`" --merge-output-format mp4 -o `"$trailerPath`" --no-warnings --quiet"

            # Add cookies from browser to handle age-restricted videos
            if ($script:Config.YtDlpCookieBrowser) {
                $ytdlpArgs += " --cookies-from-browser $($script:Config.YtDlpCookieBrowser)"
            }
    ...
    foreach ($folder in $movieFolders) {
        ...
        foreach ($file in $allFiles) {
            ...
            if ($file.Extension -in $script:Config.VideoExtensions) {
                ...
                elseif ($file.Name -match 'sam?ple|sampe|smaple') {
                    $shouldDelete = $true
                    $reason = "sample video"
                }
            }
...
function Invoke-MetadataRefresh {
    ...
    if ($MediaType -eq "Movies") {
        ...
        foreach ($folder in $foldersNeedingWork) {
            ...
            # Search TMDB (retry without year if first attempt fails)
            $searchResult = Search-TMDBMovie -Title $title -Year $year -ApiKey $script:Config.TMDBApiKey
            if (-not $searchResult -and $year) {
                $searchResult = Search-TMDBMovie -Title $title -ApiKey $script:Config.TMDBApiKey
            }
        ...
        # Display corrected folders
        if ($updatedFolders.Count -gt 0) {
            Write-Host "`n--- Folders Corrected ---" -ForegroundColor Green
            foreach ($name in $updatedFolders) {
        ...
        if ($updatedFolders.Count -gt 0) {
            Write-Host "`n--- Shows Corrected ---" -ForegroundColor Green
            foreach ($name in $updatedFolders) {
                Write-Host "  $name" -ForegroundColor White
            }
        }
    ...
    try {
        ...
        if ($Metadata.Actors -and $Metadata.Actors.Count -gt 0) {
            ...
            if ($actorsWithThumbs.Count -gt 0) {
                ...
                foreach ($actor in $actorsWithThumbs) {
                    # Sanitize actor name for filename. Spaces -> underscores
                    # because Kodi expects "Brad_Pitt.jpg" not "Brad Pitt.jpg"
                    # for its NFO-actor-name -> image-file lookup.
                    $safeActorName = $actor.Name -replace '[\\/:*?"<>|\s]', '_'
                    $actorImagePath = Join-Path $actorsFolder "$safeActorName.jpg"
...
<#
.SYNOPSIS
    Removes duplicate video files within the same folder, keeping the best quality
.PARAMETER Path
    The root path containing movie folders to check
.DESCRIPTION
    When a folder contains multiple video files of the same movie (e.g., a regular release
    and a Criterion edition), compares quality scores and keeps only the best version.
    Also cleans up orphaned NFO/subtitle files that matched the removed video.
#>
function Remove-DuplicateVideoFiles {
    param(
        [string]$Path
    )

    Write-Host "`n--- Duplicate Video Detection ---" -ForegroundColor Cyan
    Write-Log "Starting duplicate video detection in: $Path" "INFO"
    ...
    foreach ($folder in $folders) {
        ...
        # Sort by quality score descending, then by file size descending as tiebreaker
        $sorted = $scored | Sort-Object -Property Score, { $_.File.Length } -Descending
        $keeper = $sorted[0]
        $removals = $sorted | Select-Object -Skip 1

        Write-Host "  $($folder.Name)" -ForegroundColor White
        Write-Host "    Keep:   $($keeper.File.Name) (score: $($keeper.Score), $(Format-FileSize $keeper.File.Length))" -ForegroundColor Green

        foreach ($removal in $removals) {
            Write-Host "    Remove: $($removal.File.Name) (score: $($removal.Score), $(Format-FileSize $removal.File.Length))" -ForegroundColor DarkGray
            ...
            Remove-Item -LiteralPath $removal.File.FullName -Force -ErrorAction SilentlyContinue

            # Remove matching NFO file
            $nfoPath = Join-Path $folder.FullName "$([System.IO.Path]::GetFileNameWithoutExtension($removal.File.Name)).nfo"
            if (Test-Path -LiteralPath $nfoPath) {
                Remove-Item -LiteralPath $nfoPath -Force -ErrorAction SilentlyContinue
            }

            # Remove matching subtitle files
            $subBaseName = [System.IO.Path]::GetFileNameWithoutExtension($removal.File.Name)
            Get-ChildItem -LiteralPath $folder.FullName -File -ErrorAction SilentlyContinue |
                Where-Object {
                    ...
                    $_.BaseName -like "$subBaseName*"
                } | ForEach-Object {
                    Remove-Item -LiteralPath $_.FullName -Force -ErrorAction SilentlyContinue
                }

            Write-Log "Removed duplicate video: $($removal.File.Name) from $($folder.Name)" "INFO"
        }

        $foldersFixed++
    }

    if ($foldersFixed -eq 0) {
        Write-Host "  No duplicate videos found" -ForegroundColor Gray
    } else {
        Write-Host "`n  Cleaned up $foldersFixed folder(s) with duplicate videos" -ForegroundColor Green
    }
}
...
<#
.SYNOPSIS
    Renames a folder, merging contents if the target already exists
.PARAMETER SourceFolder
    The folder to rename (full path)
.PARAMETER NewName
    The new folder name (not a full path)
.DESCRIPTION
    If the target folder already exists, merges the source folder contents into it.
    When both folders contain video files, keeps the higher quality version.
    Returns the path of the final folder (renamed or merged target).
#>
function Rename-OrMergeFolder {
    param(
        [Parameter(Mandatory=$true)]
        [string]$SourceFolder,
        [Parameter(Mandatory=$true)]
    ...
    if (-not (Test-Path -LiteralPath $targetPath)) {
        Rename-Item -LiteralPath $SourceFolder -NewName $NewName -ErrorAction Stop
        return $targetPath
    }

    # Target exists - need to merge
    Write-Host "    Merging duplicate: '$([System.IO.Path]::GetFileName($SourceFolder))' into '$NewName'" -ForegroundColor DarkYellow
    Write-Log "Merging folder '$([System.IO.Path]::GetFileName($SourceFolder))' into existing '$NewName'" "INFO"

    # Check for video file conflicts
    $sourceVideos = Get-ChildItem -LiteralPath $SourceFolder -File -ErrorAction SilentlyContinue |
        Where-Object { $script:Config.VideoExtensions -contains $_.Extension.ToLower() }
    $targetVideos = Get-ChildItem -LiteralPath $targetPath -File -ErrorAction SilentlyContinue |
        Where-Object { $script:Config.VideoExtensions -contains $_.Extension.ToLower() }

    if ($sourceVideos -and $targetVideos) {
        ...
        if ($sourceQuality.Score -gt $targetQuality.Score) {
            ...
            # Remove source video files, keep target
            foreach ($sv in $sourceVideos) {
                Remove-Item -LiteralPath $sv.FullName -Force -ErrorAction SilentlyContinue
            }
        }
    }

    # Move remaining files from source to target (skip files that already exist in target)
    $sourceFiles = Get-ChildItem -LiteralPath $SourceFolder -File -ErrorAction SilentlyContinue
    ...
    # Move subdirectories from source to target
    $sourceDirs = Get-ChildItem -LiteralPath $SourceFolder -Directory -ErrorAction SilentlyContinue
    foreach ($dir in $sourceDirs) {
        $destDir = Join-Path $targetPath $dir.Name
        if (-not (Test-Path -LiteralPath $destDir)) {
            Move-Item -LiteralPath $dir.FullName -Destination $targetPath -Force -ErrorAction SilentlyContinue
        }
    }

    # Remove the now-empty source folder
    Remove-Item -LiteralPath $SourceFolder -Recurse -Force -ErrorAction SilentlyContinue
    Write-Log "Merged and removed source folder: $([System.IO.Path]::GetFileName($SourceFolder))" "INFO"
...
function Rename-CleanFolderNames {
    ...
    try {
        # Pre-pass A: heal folders that have "(YYYY)" appended at the END but
        # whose title still contains the same year + release tags. These were
        # produced by an earlier run of the post-NFO bare-year-wrap step (which
        # appends the year to the folder name without cleaning the title) on
        # folders the tag-strip pass had already skipped (no anchor present).
        # Result: "Donnie Darko 2001 Directors Cut UHD BluRay... (2001)" — the
        # tag-strip ignores it (there's nothing after the trailing parens) and
        # the dirty title gets fed to TMDB. Heal by detecting the same year
        # embedded in the title and re-cropping to the real title.
        $legacyTagPattern = '(?i)(1080p|720p|2160p|480p|4K|UHD|WEB-?DL|WEBRip|HDTV|BluRay|BDRip|BRRip|HDRip|DVDRip|x264|x265|H\.?264|H\.?265|HEVC|AAC|AC3|DDP|DTS|Atmos|REMUX|HDR|DoVi|DolbyVision|YTS|RARBG|YIFY|AMZN|NF|DSNP|HMAX|ATVP|PROPER|REPACK|Directors?\s*Cut|Extended|Unrated|Remastered|incl|Commentary|Uncut|IMAX|Anniversary)'
        Get-ChildItem -Path $Path -Directory -ErrorAction SilentlyContinue |
            Where-Object { $_.Name -match '^.+\s*\((?:19|20)\d{2}\)\s*$' } |
            ForEach-Object {
                try {
                    $original = $_.Name
                    if ($original -match '^(.+?)\s*\(((?:19|20)\d{2})\)\s*$') {
                        $coreTitle = $Matches[1].Trim()
                        $yearStr   = $Matches[2]

                        # Find the SAME year embedded in the title — last
                        # occurrence so titles that legitimately contain the
                        # year (Fantasia 2000, Blade Runner 2049) survive when
                        # the embedded year is the title-year, not a release tag.
                        $lastIdx = $coreTitle.LastIndexOf($yearStr)
                        if ($lastIdx -gt 0) {
                            $tail = $coreTitle.Substring($lastIdx + 4).Trim()
                            # Only re-crop when the tail past the embedded year
                            # actually contains release-tag tokens — otherwise
                            # the embedded year is part of the title and we'd
                            # be deleting it.
                            if ($tail -and $tail -match $legacyTagPattern) {
                                $realTitle = $coreTitle.Substring(0, $lastIdx).TrimEnd(' ', '.', '-', '_')
                                if ($realTitle) {
                                    $newName = "$realTitle ($yearStr)"
                                    if ($script:Config.DryRun) {
                                        Write-Host "[DRY-RUN] Would re-crop dirty title: '$original' -> '$newName'" -ForegroundColor Yellow
                                        Write-Log "Would re-crop dirty title: '$original' -> '$newName'" "DRY-RUN"
                                    } else {
                                        $null = Rename-OrMergeFolder -SourceFolder $_.FullName -NewName $newName
                                        Write-Log "Re-cropped dirty title: '$original' -> '$newName'" "INFO"
                    ...
                    Write-Host "Warning: Could not re-crop $($_.Name): $_" -ForegroundColor Yellow
                    Write-Log "Error re-cropping $($_.Name): $_" "ERROR"
                }
            }

        # Pre-pass B: wrap bare in-name years in parens. The tag-stripping pass
        # later in this function refuses to strip anything from a folder that
        # lacks a "(YYYY)" anchor (otherwise tags like "iT" would clobber real
        # title words). Folders like "Finding Neverland 2004 1080p BluRay..."
        # therefore survive this function unchanged and feed their full raw
        # name to TMDB. Wrapping the year here installs the anchor, so the
        # downstream tag-stripping pass can clean the tail in the same run
        # rather than waiting for a second invocation.
        # Greedy regex (`.+`, not `.+?`) anchors on the LAST year so titles
        # that contain a year — "Fantasia 2000" (2000), "Blade Runner 2049"
        # (2017) — parse correctly when the release also tags the year.
        Get-ChildItem -Path $Path -Directory -ErrorAction SilentlyContinue |
            Where-Object {
                $_.Name -notmatch '\((?:19|20)\d{2}\)' -and
                $_.Name -match '^.+[\s\.](?:19|20)\d{2}[\s\.].+$'
            } |
            ForEach-Object {
                try {
                    $original = $_.Name
                    if ($original -match '^(.+)[\s\.]((?:19|20)\d{2})[\s\.](.+)$') {
                        $titlePart = ($Matches[1] -replace '\.', ' ').Trim()
                        $yearPart  = $Matches[2]
                        $tail      = $Matches[3]
                        if (-not $titlePart) { return }
                        $newName = "$titlePart ($yearPart) $tail"
                        if ($script:Config.DryRun) {
                            Write-Host "[DRY-RUN] Would wrap bare year: '$original' -> '$newName'" -ForegroundColor Yellow
                            Write-Log "Would wrap bare year: '$original' -> '$newName'" "DRY-RUN"
                        } else {
                            $null = Rename-OrMergeFolder -SourceFolder $_.FullName -NewName $newName
                            Write-Log "Wrapped bare year: '$original' -> '$newName'" "INFO"
                            $script:Stats.FoldersRenamed++
                        }
                    }
                } catch {
                    Write-Host "Warning: Could not wrap year for $($_.Name): $_" -ForegroundColor Yellow
                    Write-Log "Error wrapping bare year for $($_.Name): $_" "ERROR"
                }
            }
...
function Rename-VideoToMatchFolder {
    ...
    try {
        ...
        foreach ($folder in $folders) {
            ...
            # If video already matches, check for double-named and orphaned associated files
            if ($currentBaseName -eq $expectedBaseName) {
                # Fix double-named files — detects both full duplication ("Movie (2024) Movie (2024).nfo")
                # and partial duplication ("Terminator 3- Rise of the Machines (2003)- Rise of the Machines (2003).nfo")
                # Strategy: video is already correct, so use it as the anchor. For each non-video file,
                # build the correct name from the folder basename + the file's real suffix.
                # The real suffix is extracted by scanning backwards from the end of the filename using
                # a whitelist of known extensions — this avoids regex confusion with dots in titles
                # (e.g., "When Harry Met Sally..." or abbreviations).
                $escapedExpected = [WildcardPattern]::Escape($expectedBaseName)
                # Known extensions for associated files (not video — those are handled separately)
                $knownExtensions = @('.nfo', '.srt', '.sub', '.idx', '.ass', '.ssa', '.vtt',
                    '.jpg', '.jpeg', '.png', '.webp', '.bmp', '.tbn',
                    '.webm', '.mp4', '.mkv', '.nfo-orig', '.bak', '.txt', '.json')
                $specialPrefixes = @('keyart', 'banner', 'poster', 'fanart', 'trailer', 'theme', 'landscape', 'clearlogo', 'disc')

                $nonVideoFiles = @(Get-ChildItem -LiteralPath $folder.FullName -File -ErrorAction SilentlyContinue |
                    Where-Object {
                        $_.Name.StartsWith($expectedBaseName, [System.StringComparison]::OrdinalIgnoreCase) -and
                        $script:Config.VideoExtensions -notcontains $_.Extension.ToLower() -and
                        $_.Name -ne "$expectedBaseName$($videoFile.Extension)"
                    })

                foreach ($file in $nonVideoFiles) {
                    $afterBase = $file.Name.Substring($expectedBaseName.Length)

                    # Build the real suffix by scanning from the end of $afterBase
                    # Step 1: Find the final extension (e.g., ".srt", ".jpg", ".nfo")
                    $suffix = $null
                    $remaining = $afterBase
                    if ($remaining -match '(\.\w{2,4})$') {
                        $finalExt = $Matches[1].ToLower()
                        if ($knownExtensions -contains $finalExt) {
                            $suffix = $Matches[1]
                    ...
                    # Step 4: Check for special prefix like -keyart, -poster, -trailer
                    if ($remaining -match '-(keyart|banner|poster|fanart|trailer|theme|landscape|clearlogo|disc)$') {
                        $suffix = "-$($Matches[1])$suffix"
                        $remaining = $remaining.Substring(0, $remaining.Length - $Matches[1].Length - 1)
                    }

                    # If we couldn't determine a known suffix, skip this file (don't guess)
                    ...
                    if ($newName -ne $file.Name) {
                        if (Test-Path -LiteralPath $newPath) {
                            # Correct file already exists — delete the double-named duplicate
                            if ($WhatIf -or $script:Config.DryRun) {
                                Write-Host "  [DRY-RUN] DELETE $($file.Name) (correct file exists)" -ForegroundColor Yellow
                            } else {
                                Remove-Item -LiteralPath $file.FullName -Force -ErrorAction SilentlyContinue
                                Write-Host "  [DEL] $($file.Name) (duplicate of $newName)" -ForegroundColor DarkGray
                            }
                        } else {
                            if ($WhatIf -or $script:Config.DryRun) {
                                Write-Host "  [DRY-RUN] $($file.Name)" -ForegroundColor Yellow
                                ...
                                Write-Host "  [FIX] $($file.Name) -> $newName" -ForegroundColor DarkGray
                            }
                        }
                        $renamed++
                    }
                ...
                if ($oldBaseName -and $oldBaseName -ne $expectedBaseName) {
                    $escapedOldBase = [WildcardPattern]::Escape($oldBaseName)
                    $orphanedFiles = @(Get-ChildItem -LiteralPath $folder.FullName -File -ErrorAction SilentlyContinue |
                        Where-Object {
                            ($_.Name -like "$escapedOldBase.*" -or $_.Name -like "$escapedOldBase-*") -and
                            $script:Config.VideoExtensions -notcontains $_.Extension.ToLower() -and
                            # Skip files already starting with the correct basename — they're not orphans
                            -not $_.Name.StartsWith($expectedBaseName, [System.StringComparison]::OrdinalIgnoreCase)
                        })
            ...
            if ($WhatIf -or $script:Config.DryRun) {
                ...
                try {
                    ...
                    $escapedBaseName = [WildcardPattern]::Escape($currentBaseName)
                    $associatedFiles = Get-ChildItem -LiteralPath $folder.FullName -File -ErrorAction SilentlyContinue |
                        Where-Object {
                            ...
                            ($baseName -eq $currentBaseName -or $_.Name -like "$escapedBaseName.*" -or $_.Name -like "$escapedBaseName-*") -and
...
function Get-EpisodeInfo {
    ...
    # Exclude codec patterns like H.264, H.265, x264, x265
    elseif ($baseName -match '^(.+?)[.\s_-]+(\d)(\d{2})[.\s_-](.*)$' -and
            $baseName -notmatch '[Hx][.\s_-]?\d{3}') {
        $info.ShowTitle = $Matches[1] -replace '\.', ' ' -replace '\s+', ' '
        $info.Season = [int]$Matches[2]
        $info.Episode = [int]$Matches[3]
        $info.Episodes += [int]$Matches[3]
...
<#
.SYNOPSIS
    Detects whether a folder contains a movie or TV show
.PARAMETER FolderPath
    Path to the folder to classify
.OUTPUTS
    String: "Movie", "TVShow", or "Unknown"
#>
function Get-MediaType {
    ...
    # Check 1: Season subfolders (strong TV indicator)
    $seasonFolders = Get-ChildItem -LiteralPath $FolderPath -Directory -ErrorAction SilentlyContinue |
    ...
    if ($folderName -match '[Ss]\d{1,2}[Ee]\d{1,2}' -or
        $folderName -match '\bSeason\s*\d' -or
        $folderName -match '\b[Ss]\d{1,2}\b(?!\d)' -or
        $folderName -match '\bComplete\s+(Series|Season)\b') {
        return "TVShow"
    }
    ...
    foreach ($file in $videoFiles) {
        $episodeInfo = Get-EpisodeInfo -FileName $file.Name
        if ($episodeInfo.Season -ne $null -and $episodeInfo.Episode -ne $null) {
    ...
    if (-not $hasYearPattern -and $videoFiles) {
        foreach ($file in $videoFiles) {
            if ($file.BaseName -match '\(?\b(19|20)\d{2}\b\)?') {
                $hasYearPattern = $true
                break
            }
        }
...
function Invoke-MovieProcessing {
    ...
    # Step 1b: TMDB health check — bulk runs become useless if TMDB is rejecting
    # every call upfront (bad key, rate limit, outage). Probing once before the
    # main work tells the user immediately whether to fix the environment vs.
    # accept basic-NFO fallbacks for everything.
    if ($script:Config.TMDBApiKey) {
        Write-Host ""
        Write-Host "Checking TMDB connectivity..." -ForegroundColor Gray -NoNewline
        $tmdbHealth = Test-TMDBHealth -ApiKey $script:Config.TMDBApiKey
        if ($tmdbHealth.Healthy) {
            Write-Host " OK" -ForegroundColor Green -NoNewline
            Write-Host " (sample lookup: $($tmdbHealth.SampleHit))" -ForegroundColor DarkGray
        } else {
            Write-Host " FAILED" -ForegroundColor Red
            Write-Host "  Reason: $($tmdbHealth.Reason)" -ForegroundColor Yellow
            $configState = if ($tmdbHealth.ConfigEndpointOk) { 'OK' } else { 'FAIL' }
            $searchState = if ($tmdbHealth.SearchEndpointOk) { 'OK' } else { 'FAIL' }
            Write-Host "  Config endpoint: $configState   Search endpoint: $searchState" -ForegroundColor Yellow
            Write-Log "TMDB health check failed: $($tmdbHealth.Reason)" "ERROR"
            Write-Host ""
            Write-Host "  Continuing will create basic NFOs (no TMDB metadata) for everything." -ForegroundColor Yellow
            $continue = Read-Host "  Continue anyway? (Y/N) [N]"
            if ($continue -notmatch '^[Yy]') {
                Write-Host "Aborted by user." -ForegroundColor Gray
                Write-Log "Movie processing aborted after TMDB health check failure" "WARN"
                return
            }
            Write-Log "Proceeding with movie processing despite TMDB failure" "WARN"
        }
    }
    ...
    # Step 8c: Remove empty folders (left behind by subtitle moves, extractions, etc.)
    ...
    # Step 12: Transfer to main library (if this is an inbox and library path is configured)
    if ($script:Config.MoviesLibraryPath -and (Test-Path $script:Config.MoviesLibraryPath)) {
        ...
        if ($isInbox) {
            ...
            if ($movieCount -gt 0) {
                ...
                # Check for NFO validation failures first (these are serious)
                $transferBlocked = $false
                if ($script:Stats.NFOValidationFailures -gt 0) {
                    Write-Host ""
                ...
                $otherErrors = $script:Stats.Errors - $script:Stats.NFOValidationFailures
                if (-not $transferBlocked -and $otherErrors -gt 0) {
                    Write-Host ""
                    Write-Host "WARNING: $otherErrors error(s) occurred during processing:" -ForegroundColor Red
                    ...
                    if ($errorConfirm -ne 'Y' -and $errorConfirm -ne 'y') {
                        Write-Host "Transfer skipped - fix errors and re-process, or transfer manually." -ForegroundColor Yellow
                        Write-Log "Library transfer skipped by user due to $otherErrors processing error(s)" "WARNING"
                ...
                if (-not $transferBlocked) {
                    ...
                    if ($remainingCount -gt 0) {
                        ...
                        if ($script:SessionAutoMove -eq $true -and $otherErrors -eq 0) {
                            ...
                            $transferResult = Move-MoviesToLibrary -InboxPath $Path -LibraryPath $script:Config.MoviesLibraryPath
                        ...
                        # If anything actually moved into the library AND the
                        # user has an MSIF folder configured, refresh movie set
                        # artwork. Scans the full library (not just newly moved
                        # folders) because a single new arrival can complete a
                        # collection whose existing members already need MSIF
                        # entries; per-folder skip-if-present keeps repeat work
                        # cheap.
                        if ($transferResult -and $transferResult.Moved -gt 0 -and
                            $script:Config.MovieSetArtworkPath -and
                            $script:Config.TMDBApiKey) {
                            if (Test-Path -LiteralPath $script:Config.MovieSetArtworkPath) {
                                Write-Host ""
                                Write-Host "Refreshing movie set artwork for new arrivals..." -ForegroundColor Cyan
                                try {
                                    Invoke-MovieSetArtworkDownload `
                                        -MoviesPath $script:Config.MoviesLibraryPath `
                                        -SetArtworkPath $script:Config.MovieSetArtworkPath
                                } catch {
                                    Write-Host "Movie set artwork refresh failed: $_" -ForegroundColor Yellow
                                    Write-Log "Movie set artwork refresh failed after inbox processing: $_" "WARN"
                                }
                            } else {
                                Write-Host ""
                                Write-Host "Skipping movie set artwork — MSIF path not reachable: $($script:Config.MovieSetArtworkPath)" -ForegroundColor DarkYellow
                                Write-Log "MSIF path not reachable after inbox processing: $($script:Config.MovieSetArtworkPath)" "WARN"
                            }
                        }
                    }
                }
    ...
    } elseif ($script:Config.MoviesLibraryPath -and -not (Test-Path $script:Config.MoviesLibraryPath)) {
        # Library path is configured but not reachable
        Write-Host "`n--- Transfer to Library ---" -ForegroundColor Cyan
        Write-Host "Library path not reachable: $($script:Config.MoviesLibraryPath)" -ForegroundColor Red
        Write-Host "Movies will remain in the inbox. Check that the drive is mounted." -ForegroundColor Yellow
        Write-Log "Library transfer skipped - path not reachable: $($script:Config.MoviesLibraryPath)" "WARNING"
...
function Invoke-TVShowProcessing {
    ...
    } elseif ($script:Config.TVShowsLibraryPath -and -not (Test-Path $script:Config.TVShowsLibraryPath)) {
        # Library path is configured but not reachable
        Write-Host "`n--- Transfer to Library ---" -ForegroundColor Cyan
        Write-Host "Library path not reachable: $($script:Config.TVShowsLibraryPath)" -ForegroundColor Red
        Write-Host "TV shows will remain in the inbox. Check that the drive is mounted." -ForegroundColor Yellow
        Write-Log "Library transfer skipped - path not reachable: $($script:Config.TVShowsLibraryPath)" "WARNING"
...
<#
.SYNOPSIS
    Processes an inbox folder end-to-end: classify, configure, stage, process, transfer, cleanup
.DESCRIPTION
    Shared orchestration for inbox processing used by both the Process Inbox menu
    and seedbox post-download. Handles media type detection, settings confirmation,
    staging, processing, retry/undo, and cleanup.
.PARAMETER InboxPath
    The path to the inbox folder containing downloaded media
.PARAMETER SkipPrompts
    If set, uses current config defaults without prompting for settings customization
#>
function Invoke-InboxProcessing {
    param(
        [string]$InboxPath,
        [switch]$SkipPrompts
    )

    # Scan and classify inbox content
    $containerNames = @{
        '_Movies'   = 'Movie'
        '_Shows'    = 'TVShow'
        'Movies'    = 'Movie'
    ...
    foreach ($folder in $topFolders) {
        if ($containerNames.ContainsKey($folder.Name)) {
            $preClassified = $containerNames[$folder.Name]
            $children = Get-ChildItem -LiteralPath $folder.FullName -Directory -ErrorAction SilentlyContinue
    ...
    # Pull _Review children back into the run. _Review holds folders that the
    # transfer step quarantined for human attention (bad NFOs, mismatch). Once
    # the user has fixed metadata or moved the folder back, the next routine
    # run should reprocess them — this block is what makes that automatic.
    # Each child is classified independently because _Review can hold a mix
    # of movies and TV shows.
    $reviewPath = Join-Path $InboxPath "_Review"
    if (Test-Path -LiteralPath $reviewPath -PathType Container) {
        $reviewChildren = @(Get-ChildItem -LiteralPath $reviewPath -Directory -ErrorAction SilentlyContinue)
        if ($reviewChildren.Count -gt 0) {
            Write-Host "Including $($reviewChildren.Count) folder(s) from _Review for retry" -ForegroundColor DarkYellow
    ...
    Write-Host "Found $($allFolders.Count) folder(s). Classifying..." -ForegroundColor White

    # Display classification summary
    Write-Host ""
    if ($movieFolders.Count -gt 0) {
        Write-Host "  Movies:  " -NoNewline -ForegroundColor Cyan
        Write-Host ($movieFolders.Name -join ", ")
    }
    if ($tvFolders.Count -gt 0) {
        Write-Host "  TV Shows:" -NoNewline -ForegroundColor Magenta
        Write-Host " $($tvFolders.Name -join ', ')"
    }
    ...
    $dryRunDisplay = if ($script:Config.DryRun) { "Yes" } else { "No" }
    ...
    if (-not $SkipPrompts) {
        $useDefaults = Read-Host "`nUse these settings? (Y/N) [Y]"

        if ($useDefaults -match '^[Nn]') {
            # Walk through detailed configuration
            $dryRunInput = Read-Host "`nEnable dry-run mode? (Y/N) [N]"
            ...
            $nfoInput = Read-Host "Generate Kodi NFO files? (Y/N) [$nfoDefault]"
            if ($nfoInput -match '^[Yy]') {
            ...
            if ($movieFolders.Count -gt 0) {
                ...
                if ($dupeInput -match '^[Yy]') {
                    $script:Config.CheckDuplicates = $true
                    Write-Host "Duplicate detection enabled" -ForegroundColor Green
                } else {
                    $script:Config.CheckDuplicates = $false
                }
            }

            # Trailer downloads
            if (Test-YtDlpInstallation -and $movieFolders.Count -gt 0) {
                $trailerDefault = if ($script:Config.DownloadTrailers) { "Y" } else { "N" }
                $trailerInput = Read-Host "Download movie trailers from YouTube? (Y/N) [$trailerDefault]"
                if ($trailerInput -match '^[Yy]') {
            ...
            # Subtitle mode
            $currentSubModeDisplay = switch ($script:Config.SubtitleMode) { "All" { "All" }; "Foreign" { "Foreign only" }; default { "None" } }
            Write-Host "Subtitle mode: $currentSubModeDisplay" -ForegroundColor $(if ($script:Config.SubtitleMode -ne "None") { "Green" } else { "Gray" })
            $changeSubMode = Read-Host "Change subtitle mode? (Y/N) [N]"
            if ($changeSubMode -match '^[Yy]') {
                Write-Host "  1. All (always download)"
                ...
                switch ($subModeChoice) {
                    "1" { $script:Config.SubtitleMode = "All"; Write-Host "Subtitles: All ($($script:Config.SubtitleLanguage))" -ForegroundColor Green }
                    "2" { $script:Config.SubtitleMode = "Foreign"; Write-Host "Subtitles: Foreign only ($($script:Config.SubtitleLanguage))" -ForegroundColor Green }
                    default { $script:Config.SubtitleMode = "None"; Write-Host "Subtitle downloads disabled" -ForegroundColor Cyan }
                }
            }
            ...
            if ($tvFolders.Count -gt 0) {
                ...
                if ($renameInput -match '^[Yy]$') {
                    ...
                    Write-Host "Episodes will be renamed" -ForegroundColor Green
                } elseif ($renameInput -match '^[Nn]$') {
                    $script:Config.RenameEpisodes = $false
                    Write-Host "Episode filenames preserved" -ForegroundColor Cyan
                }
            }
        } else {
            # Using defaults - set auto-move based on library paths
            if ($script:Config.MoviesLibraryPath -or $script:Config.TVShowsLibraryPath) {
                $script:SessionAutoMove = $true
            }
        }
    } else {
        # SkipPrompts mode - use defaults silently
        if ($script:Config.MoviesLibraryPath -or $script:Config.TVShowsLibraryPath) {
            $script:SessionAutoMove = $true
        }
    }
    ...
    $moviesStagingPath = Join-Path $InboxPath "_staging_movies"
    $tvStagingPath = Join-Path $InboxPath "_staging_tvshows"

    if ($movieFolders.Count -gt 0) {
        New-Item -Path $moviesStagingPath -ItemType Directory -Force -ErrorAction SilentlyContinue | Out-Null
    }
    if ($tvFolders.Count -gt 0) {
        New-Item -Path $tvStagingPath -ItemType Directory -Force -ErrorAction SilentlyContinue | Out-Null
    }

    # Move classified folders to staging
    foreach ($folder in $movieFolders) {
        $dest = Join-Path $moviesStagingPath $folder.Name
        Move-Item -LiteralPath $folder.FullName -Destination $dest -ErrorAction SilentlyContinue
    }
    foreach ($folder in $tvFolders) {
        $dest = Join-Path $tvStagingPath $folder.Name
        Move-Item -LiteralPath $folder.FullName -Destination $dest -ErrorAction SilentlyContinue
    }

    # Save original config values and temporarily set inbox paths for library transfer
    $originalMoviesInbox = $script:Config.MoviesInboxPath
    $originalTVInbox = $script:Config.TVShowsInboxPath
    $script:Config.MoviesInboxPath = $moviesStagingPath
    $script:Config.TVShowsInboxPath = $tvStagingPath

    # Process Movies
    if ($movieFolders.Count -gt 0) {
        Write-Host "`n=== Processing Movies ($($movieFolders.Count)) ===" -ForegroundColor Magenta
        $script:ProcessedPath = $moviesStagingPath
        $script:ProcessedMediaType = "Movies"

        Invoke-MovieProcessing -Path $moviesStagingPath

        Invoke-RetryFailedOperations
        Save-UndoManifest

        if ($script:Config.DryRun) {
            Export-DryRunReport -Path $moviesStagingPath -MediaType "Movies"
        }
    }

    # Process TV Shows
    if ($tvFolders.Count -gt 0) {
        Write-Host "`n=== Processing TV Shows ($($tvFolders.Count)) ===" -ForegroundColor Magenta
        $script:ProcessedPath = $tvStagingPath
        $script:ProcessedMediaType = "TVShows"

        Invoke-TVShowProcessing -Path $tvStagingPath

        Invoke-RetryFailedOperations
        Save-UndoManifest

        if ($script:Config.DryRun) {
            Export-DryRunReport -Path $tvStagingPath -MediaType "TVShows"
        }
    }

    # Restore original config values
    $script:Config.MoviesInboxPath = $originalMoviesInbox
    $script:Config.TVShowsInboxPath = $originalTVInbox

    # Move any remaining items from staging back to inbox
    foreach ($stagingDir in @($moviesStagingPath, $tvStagingPath)) {
        if (Test-Path $stagingDir) {
            $remaining = Get-ChildItem -LiteralPath $stagingDir -Directory -ErrorAction SilentlyContinue
            foreach ($item in $remaining) {
                Move-Item -LiteralPath $item.FullName -Destination (Join-Path $InboxPath $item.Name) -ErrorAction SilentlyContinue
            }
            # Clean up empty staging dirs
            $stageFiles = Get-ChildItem -LiteralPath $stagingDir -Recurse -ErrorAction SilentlyContinue
    ...
    if ($unknownFolders.Count -gt 0) {
        Write-Host "`n--- Skipped Folders ---" -ForegroundColor Yellow
        Write-Host "The following $($unknownFolders.Count) folder(s) could not be classified as Movie or TV Show:" -ForegroundColor Yellow
        foreach ($folder in $unknownFolders) {
            Write-Host "  - $($folder.Name)" -ForegroundColor DarkGray
        }
        Write-Host "Tip: Ensure names contain a year (movies) or episode pattern like S01E01 (TV shows)." -ForegroundColor DarkGray
    }
...
if ($runSetup) {
    ...
    # Inbox folder (single shared inbox for all downloads)
    $defaultInbox = if ($script:Config.InboxPath) { $script:Config.InboxPath } elseif ($script:Config.MoviesInboxPath) { $script:Config.MoviesInboxPath } else { "G:\Inbox" }
    $inbox = Read-Host "Inbox folder (for all downloads) [$defaultInbox]"
    if (-not $inbox) { $inbox = $defaultInbox }
    $script:Config.InboxPath = $inbox
    ...
    if ($setupSftp -match '^[Yy]') {
        ...
        if ($sftpHost) {
            ...
            $defaultLocal = if ($script:Config.InboxPath) { Split-Path $script:Config.InboxPath -Parent } else { "G:" }
            $localPath = Read-Host "  Local download folder [$defaultLocal]"
            if ($localPath) {
                $script:Config.SFTPLocalPath = $localPath
            } else {
                $script:Config.SFTPLocalPath = $defaultLocal
            }
...
switch ($type) {
    "1" {
        ...
        Write-Host "`n=== Process Inbox ===" -ForegroundColor Cyan

        # Determine inbox path
        $inboxPath = $null
        if ($script:Config.InboxPath -and (Test-Path $script:Config.InboxPath)) {
            $inboxPath = $script:Config.InboxPath
            Write-Host "Inbox: $inboxPath" -ForegroundColor Green
        }

        if (-not $inboxPath) {
            $inboxPath = Select-FolderDialog -Description "Select your inbox folder (all downloads)"
            if ($inboxPath) {
                $savePath = Read-Host "Save this as your default inbox? (Y/N) [Y]"
                if ($savePath -notmatch '^[Nn]') {
                    $script:Config.InboxPath = $inboxPath
                    Export-Configuration
                    Write-Host "Inbox saved to configuration" -ForegroundColor Green
                }
            }
        }
    ...
    "2" {
        ...
        Write-Host "16. Fix Actor Image Names        " -NoNewline; Write-Host "- Rename .actors files for Kodi (Brad Pitt.jpg -> Brad_Pitt.jpg)" -ForegroundColor DarkGray
        Write-Host "17. Foreign Language Check        " -NoNewline; Write-Host "- Find movies not available in your language" -ForegroundColor DarkGray
        if ($quarantineHasContents) {
            Write-Host ""
            Write-Host "--- Quarantine ---" -ForegroundColor Yellow
            Write-Host "18. Return from Quarantine       " -NoNewline; Write-Host "- Re-test quarantined folders and move passing ones back" -ForegroundColor DarkGray
        }
        Write-Host ""
        Write-Host "0. Back to Main Menu"
        Write-Host "X. Exit"
        ...
        # Option 16: Fix Actor Image Names. Handled here so it bypasses the
        # library picker — the migration auto-iterates BOTH the movies and
        # TV libraries from config in one pass (one-shot the user shouldn't
        # need to remember to run twice).
        if ($fixChoice -eq '16') {
            $libraryRoots = @($script:Config.MoviesLibraryPath, $script:Config.TVShowsLibraryPath) | Where-Object { $_ }
            if ($libraryRoots.Count -eq 0) {
                Write-Host "No library paths configured (MoviesLibraryPath / TVShowsLibraryPath)." -ForegroundColor Red
                continue
            }
            $dryRunInput = Read-Host "Run in dry-run mode first? (Y/N) [Y]"
            $dryRun = $dryRunInput -notmatch '^[Nn]'
            if ($dryRun) {
                $pending = Repair-ActorImageNames -Path $libraryRoots -WhatIf
                Write-Host ""
                if ($pending.FilesRenamed -gt 0) {
                    $apply = Read-Host "Apply these changes? (Y/N) [N]"
                    if ($apply -match '^[Yy]') {
                        $null = Repair-ActorImageNames -Path $libraryRoots
                    } else {
        ...
        # Option 18: Return from Quarantine. Handled here so it bypasses the
        ...
        if (-not $path) {
            ...
            switch ($fixChoice) {
                ...
                "7" {
                    ...
                    # Show API key status
                    $hasTMDB = [bool]$script:Config.TMDBApiKey
                    $hasFanart = [bool]$script:Config.FanartTVApiKey
                    Write-Host ""
                    Write-Host "  TMDB API key:      " -NoNewline -ForegroundColor Gray
                    if ($hasTMDB) { Write-Host "configured" -ForegroundColor Green } else { Write-Host "not configured" -ForegroundColor Red }
                    Write-Host "    provides: poster, fanart, actor images" -ForegroundColor DarkGray
                    Write-Host "  Fanart.tv API key: " -NoNewline -ForegroundColor Gray
                    if ($hasFanart) { Write-Host "configured" -ForegroundColor Green } else { Write-Host "not configured" -ForegroundColor Yellow }
                    Write-Host "    provides: clearlogo, logo, keyart, banner, landscape, disc, clearart, extrafanart" -ForegroundColor DarkGray
                    Write-Host ""

                    Write-Host "--- Delete ---" -ForegroundColor Red
                    Write-Host "  1. Clear ALL artwork (removes everything, requires re-download)"
                    Write-Host "  2. Clear specific types (choose which to remove)"
                    Write-Host ""
                    ...
                    if ($artChoice -eq '0') {
                        Write-Host "Cancelled" -ForegroundColor Gray
                    } elseif ($artChoice -in @('1', '2')) {
                        # Clear options
                        $allTypes = @("poster", "fanart", "clearlogo", "logo", "banner", "landscape", "disc", "clearart", "keyart", "extrafanart", "actors")
                        $typesToRemove = @()

                        if ($artChoice -eq '1') {
                            $typesToRemove = $allTypes
                    ...
                    } elseif ($artChoice -in @('3', '4')) {
                        # Download options
                        $clearFirst = ($artChoice -eq '4')

                        # Check API keys
                        if (-not $hasTMDB) {
                            Write-Host "`nTMDB API key required for artwork download" -ForegroundColor Red
                            Write-Host "Configure it in Settings > Manage API Keys" -ForegroundColor Yellow
                        } else {
                            # Show what will happen
                            Write-Host ""
                            if ($clearFirst) {
                                Write-Host "This will clear all existing artwork and re-download from scratch." -ForegroundColor Yellow
                            } else {
                                Write-Host "This will download artwork for any missing types (existing files are kept)." -ForegroundColor Cyan
                            }
                            Write-Host ""
                            Write-Host "  From TMDB:      poster, fanart, actor images" -ForegroundColor White
                            if ($hasFanart) {
                                Write-Host "  From Fanart.tv: clearlogo, logo, keyart, banner, landscape, disc, clearart, extrafanart" -ForegroundColor White
                            } else {
                                Write-Host "  Fanart.tv:      skipped (no API key)" -ForegroundColor DarkYellow
                                Write-Host "                  To get clearlogo, keyart, banner, etc., add a fanart.tv key in Settings" -ForegroundColor DarkGray
                            }

                            $confirm = Read-Host "`nProceed? (Y/N) [N]"
                            if ($confirm -match '^[Yy]') {
                                # Clear first if re-downloading
                                if ($clearFirst) {
                                    $allTypes = @("poster", "fanart", "clearlogo", "logo", "banner", "landscape", "disc", "clearart", "keyart", "extrafanart", "actors")
                                    Invoke-ArtworkManagement -Path $path -Action Selective -ArtworkTypes $allTypes -Quiet
                                }

                                # Download
                                Write-Host "`n--- Downloading artwork ---" -ForegroundColor Cyan
                                $folders = Get-ChildItem -Path $path -Directory -ErrorAction SilentlyContinue |
                                    Where-Object { $_.Name -ne '_Trailers' }
                                ...
                                Write-Host "`n--- Artwork Download Complete ---" -ForegroundColor Green
                                Write-Host "  Processed: $processedCount folders" -ForegroundColor White
                        ...
                        if ($mediaType -ne 'Movies') {
                            ...
                            if ($script:Config.MovieSetArtworkPath) {
                                if (-not (Test-Path $path)) {
                                    ...
                                    $confirm = Read-Host "`nProceed? (Y/N) [N]"
                                    if ($confirm -match '^[Yy]') {
                                        # Cache speeds re-runs by remembering each folder's TMDB+collection
                                        # status keyed by NFO mtime. Force-refresh re-queries everything,
                                        # useful when TMDB has updated collection membership.
                                        $refreshAns = Read-Host "Force-refresh cache (re-query TMDB for every movie)? (Y/N) [N]"
                                        $forceRefresh = ($refreshAns -match '^[Yy]')
                                        Invoke-MovieSetArtworkDownload `
                                            -MoviesPath $path `
                                            -SetArtworkPath $script:Config.MovieSetArtworkPath `
        ...
        switch ($utilChoice) {
                ...
                while ($true) {
                    ...
                    if ($isConfigured) {
                        ...
                        if ($script:Config.SFTPLocalPath) {
                            Write-Host "Local:  $($script:Config.SFTPLocalPath)" -ForegroundColor Gray
                        }
                        Write-Host ""
                    ...
                    switch ($seedboxMenuChoice) {
                        "1" {
                            ...
                            if (-not $script:SyncModuleLoaded) {
                                ...
                                while ($true) {
                                    ...
                                    if ($script:Config.SFTPWorkingPaths.Count -gt 0) {
                                        Write-Host "Working dir:  $($script:Config.SFTPWorkingPaths -join ', ')" -ForegroundColor DarkGray
                                    }
                                    Write-Host ""
                                    ...
                                    switch ($sftpChoice) {
                                        "1" {
                                            ...
                                            if ($script:Config.SFTPPassword) {
                                                $checkParams.Password = $script:Config.SFTPPassword
                                            }
                                            if ($script:Config.SFTPPrivateKeyPath) {
                                                $checkParams.PrivateKeyPath = $script:Config.SFTPPrivateKeyPath
                                            }

                                            $result = Get-SFTPNewFiles @checkParams
                                            Write-Log "SFTP Check: $($result.NewFiles) new files found" "INFO"
                                        }
                                        "2" {
                                            # Download new files
                                            ...
                                            if (-not $localBase) {
                                                $localBase = Select-FolderDialog -Description "Select local folder for SFTP downloads"
                                                if ($localBase) {
                                                    $saveLocal = Read-Host "Save this as your default SFTP download folder? (Y/N) [Y]"
                                                    if ($saveLocal -notmatch '^[Nn]') {
                                                        $script:Config.SFTPLocalPath = $localBase
                                                        Export-Configuration
                                                        Write-Host "SFTP download folder saved" -ForegroundColor Green
                                                    }
                                                }
                                            }
                                            ...
                                            Write-Host "Download to: $localBase" -ForegroundColor Gray
                                            ...
                                            if ($result.Downloaded -gt 0 -and -not $whatIf) {
                                                ...
                                                if ($processNow -notmatch '^[Nn]') {
                                                    Invoke-InboxProcessing -InboxPath $localBase
                                                }
                                            }
                                        ...
                                        "3" {
                                            ...
                                            # rTorrent's move-on-completion plugin failed to relocate. Only
                                            # runs when a working dir is configured. Reuses the dry-run
                                            # toggle the user already chose for the regular prune.
                                            if ($script:Config.SFTPWorkingPaths -and $script:Config.SFTPWorkingPaths.Count -gt 0) {
                                                Write-Host ""
                                                # Working-dir prune uses its own age floor — defaults to 14 days
                                                ...
                                                if ($script:Config.SFTPPassword) {
                                                    $workingPruneParams.Password = $script:Config.SFTPPassword
                                                }
                                                if ($script:Config.SFTPPrivateKeyPath) {
                                                    $workingPruneParams.PrivateKeyPath = $script:Config.SFTPPrivateKeyPath
                                                }

                                                $workingResult = Invoke-SFTPPruneWorkingDir @workingPruneParams

                                                if ($workingResult.Error) {
                                                    Write-Log "SFTP Working-dir prune aborted: $($workingResult.Error)" "WARN"
                                                } elseif ($workingResult.WhatIf) {
                                                    Write-Log "SFTP Working-dir prune dry-run: $($workingResult.Eligible) folder(s) eligible" "INFO"
                                                } else {
                                                    Write-Log "SFTP Working-dir prune: $($workingResult.Deleted) deleted, $($workingResult.Failed) failed, $($workingResult.Skipped) left alone (no library match)" "INFO"
                                                }
                                            }
                                        }
                                        "4" {
                                        ...
                                        "5" {
                                            # Find incomplete downloads — scan working dir for in-progress
                                            # downloads and compare sync vs prune folder sizes
                                            Write-Log "Scanning for incomplete downloads (working dir + sync/prune size compare)" "INFO"

                                            # Offer to configure the rTorrent working dir if missing
                                            if (-not $script:Config.SFTPWorkingPaths -or $script:Config.SFTPWorkingPaths.Count -eq 0) {
                                                Write-Host ""
                                                Write-Host "--- Working Path Setup ---" -ForegroundColor Yellow
                                                Write-Host "The working path is where rTorrent writes downloads in progress" -ForegroundColor Gray
                                                Write-Host "(before ruTorrent's 'Move on completion' relocates them). Anything" -ForegroundColor Gray
                                                Write-Host "sitting here is not yet finished — that's the most reliable signal" -ForegroundColor Gray
                                                Write-Host "for an incomplete download. Common paths:" -ForegroundColor Gray
                                                Write-Host "    /home/<user>/downloads/rtorrent/" -ForegroundColor DarkGray
                                                Write-Host "    /home/<user>/downloads/incoming/" -ForegroundColor DarkGray
                                                Write-Host ""
                                                $workPath = Read-Host "Enter working path (or Enter to skip)"
                                                if ($workPath) {
                                                    $script:Config.SFTPWorkingPaths = @($workPath -split ',\s*')
                                                    Export-Configuration
                                                    Write-Host "  Saved: $($script:Config.SFTPWorkingPaths -join ', ')" -ForegroundColor Green
                                                }
                                                Write-Host ""
                                            }
                                            ...
                                            $inProgressCount = if ($result.InProgress) { $result.InProgress.Count } else { 0 }
                                            if ($result.Error) {
                                                Write-Log "Incomplete-download check aborted: $($result.Error)" "WARN"
                                            } elseif ($inProgressCount -gt 0 -or $result.Mismatches.Count -gt 0) {
                                                Write-Log "Incomplete-download check: $inProgressCount in-progress, $($result.Mismatches.Count) sync/prune size mismatch(es)" "WARN"
                                            } else {
                                                Write-Log "No incomplete downloads or size mismatches found" "INFO"
                                            }
                                        }
                                        "6" {
                                        ...
                                        "8" {
                                            # Multi-part RAR extraction (TEST entrypoint — standalone, not yet
                                            # wired into auto-sync). Scans seedbox release folders for `.r##`
                                            # chains and runs unrar remotely so the extracted .mkv ends up in
                                            # a sibling `<release>.extracted/` folder, leaving the still-seeding
                                            # torrent files alone.
                                            Write-Host ""
                                            Write-Host "This finds multi-part RAR releases on the seedbox and extracts" -ForegroundColor Yellow
                                            Write-Host "them via remote unrar. The torrent's original .r## files are not" -ForegroundColor Yellow
                                            Write-Host "touched — extraction goes to a sibling '$($script:Config.SFTPExtractedSuffix)' folder." -ForegroundColor Yellow
                                            Write-Host ""

                                            $dryAns = Read-Host "Dry-run first (scan + report only)? (Y/N) [Y]"
                                            $unrarWhatIf = (-not ($dryAns -match '^[Nn]'))

                                            # Only offer Radarr-notify if Radarr is actually configured.
                                            # When opted in, each successful extraction triggers a
                                            # DownloadedMoviesScan command so Radarr imports the .mkv
                                            # and clears the queue item that originated the download.
                                            $notifyRadarr = $false
                                            if (-not $unrarWhatIf -and $script:Config.RadarrUrl -and $script:Config.RadarrApiKey) {
                                                $notifyAns = Read-Host "After each extraction, ask Radarr to scan/import? (Y/N) [Y]"
                                                $notifyRadarr = (-not ($notifyAns -match '^[Nn]'))
                                            }

                                            $scanPaths = @()
                                            if ($script:Config.SFTPPrunePaths -and $script:Config.SFTPPrunePaths.Count -gt 0) { $scanPaths += $script:Config.SFTPPrunePaths }
                                            if ($script:Config.SFTPWorkingPaths -and $script:Config.SFTPWorkingPaths.Count -gt 0) { $scanPaths += $script:Config.SFTPWorkingPaths }
                                            $scanPaths = @($scanPaths | Where-Object { $_ } | Select-Object -Unique)

                                            if ($scanPaths.Count -eq 0) {
                                                Write-Host "  Neither SFTPPrunePaths nor SFTPWorkingPaths is configured." -ForegroundColor Red
                                                Write-Host "  RAR releases typically live in the rTorrent complete/working tree." -ForegroundColor Yellow
                                            } else {
                                                Write-Host "  Scanning: $($scanPaths -join ', ')" -ForegroundColor Gray
                                                Write-Log "SFTP RAR extraction (TEST) starting (dry-run=$unrarWhatIf, notify-radarr=$notifyRadarr)" "INFO"
                                                ...
                                                $extractParams = @{
                                                    ...
                                                    UnrarBinary = $script:Config.SFTPUnrarCommand
                                                    ExtractedSuffix = $script:Config.SFTPExtractedSuffix
                                                    WhatIf = $unrarWhatIf
                                                }
                                                if ($script:Config.SFTPPassword) { $extractParams.Password = $script:Config.SFTPPassword }
                                                if ($script:Config.SFTPPrivateKeyPath) { $extractParams.PrivateKeyPath = $script:Config.SFTPPrivateKeyPath }
                                                if ($notifyRadarr) {
                                                    $extractParams.NotifyRadarr = $true
                                                    $extractParams.RadarrUrl = $script:Config.RadarrUrl
                                                    $extractParams.RadarrApiKey = $script:Config.RadarrApiKey
                                                }

                                                $result = Invoke-SFTPExtractRarReleases @extractParams

                                                if ($result.Error) {
                                                    Write-Log "SFTP RAR extraction aborted: $($result.Error)" "WARN"
                                                } elseif ($result.WhatIf) {
                                                    Write-Log "SFTP RAR extraction dry-run: $($result.Eligible) release(s) eligible" "INFO"
                                                } else {
                        ...
                        "3" {
                            ...
                            if ($script:Config.SFTPHost) {
                                ...
                                if ($script:Config.SFTPWorkingPaths.Count -gt 0) {
                                    Write-Host "  Working dir:  $($script:Config.SFTPWorkingPaths -join ', ')" -ForegroundColor Gray
                                }
                                if ($script:Config.SFTPSpeedLimitKBps -gt 0) {
                            ...
                            if ($script:Config.SFTPHost) {
                                ...
                                $currentWorking = if ($script:Config.SFTPWorkingPaths.Count -gt 0) { $script:Config.SFTPWorkingPaths -join ', ' } else { "(not set)" }
                                $workingPath = Read-Host "Working path(s) — rTorrent in-progress [$currentWorking]"
                                if ($workingPath) {
                                    if ($workingPath -eq 'clear') {
                                        $script:Config.SFTPWorkingPaths = @()
                                    } else {
                                        $script:Config.SFTPWorkingPaths = @($workingPath -split ',\s*')
                                    }
                                }

                                $currentLocal = if ($script:Config.SFTPLocalPath) { $script:Config.SFTPLocalPath } else { "(not set)" }
                                $localPath = Read-Host "Local download folder [$currentLocal]"
                                if ($localPath) {
                                    $script:Config.SFTPLocalPath = $localPath
                                }
                ...
                if (-not $script:MirrorModuleLoaded) {
                    ...
                    if ($mirrorChoice -eq '1') {
                        ...
                        if ($script:Config.MirrorNetworkUser -and $isUnc) {
                            # Drop ALL existing connections to this server before adding ours.
                            # Windows SMB rejects multiple connections to the same server with
                            # different credentials (System error 1219). A live connection on
                            # ANY share of this server — from Explorer browsing, another tool,
                            # or our own previous run — will block our explicit-creds attempt.
                            # Enumerate the active mappings and drop every one that matches the
                            # target server, not just the specific share root we're about to use.
                            if ($shareRoot -match '^\\\\([^\\]+)') {
            ...
            "11" {
                ...
                if ($sourceChoice -eq '1') {
                    ...
                    # Filter on parsed resolution height, not quality score. Score
                    # mixes resolution with source/codec/audio bonuses, so a 480p
                    # BluRay scoring 90 would slip past a score<60 filter even
                    # though it IS below 720p. Resolution-based filtering matches
                    # the user's mental model and aligns with the codec analysis
                    # report, which also classifies by resolution.
                    $maxHeight = switch ($resChoice) {
                        "1" { 480 }
                        "3" { 1080 }
                        default { 720 }
                    }
                    ...
                    # Resolutions strictly below the cutoff are candidates.
                    # "Unknown" is treated as suspicious for the SD/HD tiers
                    # (best to surface for review) but not for "below 1080p"
                    # where it'd flag too many edge cases.
                    $resHeightOf = {
                        param($Res)
                        if ($Res -match '^(\d+)p$') { return [int]$Matches[1] }
                        return $null
                    }
                    ...
                    # Strengthened filter (matches Quality.psm1): excludes
                    # trailers/samples/extras across separator styles
                    # (`-trailer.`, `.trailer.`, `_trailer.`, bare `trailer.`).
                    # Sort-Object Length already prefers the main feature among
                    # what's left, but with this filter we won't accidentally
                    # pick a 5 GB extras compilation over a 4 GB main movie.
                    $junkNameRegex = '(?i)(^|[\.\-_\s])(trailer|sample|featurette|behind[\.\-_\s]?the[\.\-_\s]?scenes|extras?|deleted[\.\-_\s]?scenes?)($|[\.\-_\s])'
                    foreach ($folder in $folders) {
                        $scanned++
                        $videoFile = Get-ChildItem -LiteralPath $folder.FullName -File -ErrorAction SilentlyContinue |
                            Where-Object { $script:Config.VideoExtensions -contains $_.Extension.ToLower() -and $_.Name -notmatch $junkNameRegex } |
                            Sort-Object Length -Descending | Select-Object -First 1
                        if ($videoFile) {
                            $quality = Invoke-QualityScore -FileName $videoFile.Name -FilePath $videoFile.FullName
                            $heightPx = & $resHeightOf $quality.Resolution
                            $includeForUnknown = ($maxHeight -le 720) -and ($quality.Resolution -eq 'Unknown')
                            $isBelowThreshold = ($null -ne $heightPx -and $heightPx -lt $maxHeight) -or $includeForUnknown
...
"@
    ...
    "4" {
        ...
        Write-Host "2. Manage API Keys              " -NoNewline; Write-Host "- Add, edit, or remove API keys" -ForegroundColor DarkGray
        Write-Host "3. Configure SFTP Sync          " -NoNewline; Write-Host "- Set up seedbox connection" -ForegroundColor DarkGray
        ...
        switch ($configChoice) {
            ...
            "2" {
                # Manage API Keys
                Write-Host "`n=== Manage API Keys ===" -ForegroundColor Cyan
                Write-Host "All API keys are free. They enable metadata, artwork, and subtitle downloads." -ForegroundColor Gray
                Write-Host ""

                # Show current status
                $tmdbStatus = if ($script:Config.TMDBApiKey) { "[Configured]" } else { "[Not Set]" }
                $tmdbColor = if ($script:Config.TMDBApiKey) { "Green" } else { "Yellow" }
                $tvdbStatus = if ($script:Config.TVDBApiKey) { "[Configured]" } else { "[Not Set]" }
                $tvdbColor = if ($script:Config.TVDBApiKey) { "Green" } else { "Yellow" }
                $fanartStatus = if ($script:Config.FanartTVApiKey) { "[Configured]" } else { "[Not Set]" }
                ...
                Write-Host "  2. TVDB    $tvdbStatus" -ForegroundColor $tvdbColor -NoNewline
                Write-Host "  - TV show metadata (thetvdb.com)" -ForegroundColor DarkGray
                Write-Host "  3. Fanart  $fanartStatus" -ForegroundColor $fanartColor -NoNewline
                ...
                switch ($keyChoice) {
                    "1" {
                        ...
                        if ($script:Config.TMDBApiKey) {
                            Write-Host "Current key: $($script:Config.TMDBApiKey.Substring(0, [Math]::Min(8, $script:Config.TMDBApiKey.Length)))..." -ForegroundColor Gray
                        }
                        $key = Read-Host "Enter TMDB API key (or press Enter to keep current, 'clear' to remove)"
                        if ($key -eq 'clear') {
                            $script:Config.TMDBApiKey = $null
                            Write-Host "TMDB API key removed" -ForegroundColor Yellow
                            Export-Configuration
                        } elseif ($key) {
                            Write-Host "Validating..." -ForegroundColor Gray
                            if (Test-TMDBApiKey -ApiKey $key) {
                                $script:Config.TMDBApiKey = $key
                                Write-Host "TMDB API key saved!" -ForegroundColor Green
                                Export-Configuration
                            } else {
                    ...
                    "2" {
                        Write-Host "`nGet a free API key at: https://thetvdb.com/dashboard" -ForegroundColor Yellow
                        if ($script:Config.TVDBApiKey) {
                            Write-Host "Current key: $($script:Config.TVDBApiKey.Substring(0, [Math]::Min(8, $script:Config.TVDBApiKey.Length)))..." -ForegroundColor Gray
                        }
                        $key = Read-Host "Enter TVDB API key (or press Enter to keep current, 'clear' to remove)"
                        if ($key -eq 'clear') {
                            $script:Config.TVDBApiKey = $null
                            Write-Host "TVDB API key removed" -ForegroundColor Yellow
                            Export-Configuration
                        } elseif ($key) {
                            $script:Config.TVDBApiKey = $key
                            Write-Host "TVDB API key saved!" -ForegroundColor Green
                            Export-Configuration
                        }
                    }
                    "3" {
                        Write-Host "`nGet a free API key at: https://fanart.tv/get-an-api-key/" -ForegroundColor Yellow
                        if ($script:Config.FanartTVApiKey) {
                            Write-Host "Current key: $($script:Config.FanartTVApiKey.Substring(0, [Math]::Min(8, $script:Config.FanartTVApiKey.Length)))..." -ForegroundColor Gray
                        }
                        $key = Read-Host "Enter Fanart.tv API key (or press Enter to keep current, 'clear' to remove)"
                        if ($key -eq 'clear') {
                            $script:Config.FanartTVApiKey = $null
                            Write-Host "Fanart.tv API key removed" -ForegroundColor Yellow
                            Export-Configuration
                        } elseif ($key) {
                            Write-Host "Validating..." -ForegroundColor Gray
                            if (Test-FanartTVApiKey -ApiKey $key) {
                                $script:Config.FanartTVApiKey = $key
                                Write-Host "Fanart.tv API key saved!" -ForegroundColor Green
                                Export-Configuration
                            } else {
                                Write-Host "Invalid API key" -ForegroundColor Red
                            }
                    ...
                    "4" {
                        Write-Host "`nGet a free API key at: https://subdl.com/panel/api" -ForegroundColor Yellow
                        if ($script:Config.SubdlApiKey) {
                            Write-Host "Current key: $($script:Config.SubdlApiKey.Substring(0, [Math]::Min(8, $script:Config.SubdlApiKey.Length)))..." -ForegroundColor Gray
                        }
                        $key = Read-Host "Enter Subdl API key (or press Enter to keep current, 'clear' to remove)"
                        if ($key -eq 'clear') {
                            $script:Config.SubdlApiKey = $null
                            Write-Host "Subdl API key removed" -ForegroundColor Yellow
                            Export-Configuration
                        } elseif ($key) {
                            $script:Config.SubdlApiKey = $key
                            Write-Host "Subdl API key saved!" -ForegroundColor Green
                            Export-Configuration
                        }
                    }
                    "5" {
            ...
            "3" {
                ...
                if ($script:Config.SFTPHost) {
                    ...
                    Write-Host "  Local:  $(if ($script:Config.SFTPLocalPath) { $script:Config.SFTPLocalPath } else { '(not set)' })" -ForegroundColor Gray
                    Write-Host ""
                }
            ...
            "7" {
                ...
                Write-Host "`n=== Configure Library Paths ===" -ForegroundColor Cyan
                Write-Host ""

                # Inbox
                $currentInbox = if ($script:Config.InboxPath) { $script:Config.InboxPath } else { "(not set)" }
                Write-Host "  Inbox path: " -NoNewline -ForegroundColor White
                if ($script:Config.InboxPath -and (Test-Path $script:Config.InboxPath)) {
                    Write-Host $currentInbox -ForegroundColor Green
                } elseif ($script:Config.InboxPath) {
                    Write-Host "$currentInbox (not reachable)" -ForegroundColor Red
                } else {
                    Write-Host $currentInbox -ForegroundColor Yellow
                }

                # Movies Library
                $currentMovies = if ($script:Config.MoviesLibraryPath) { $script:Config.MoviesLibraryPath } else { "(not set)" }
                Write-Host "  Movies library: " -NoNewline -ForegroundColor White
                if ($script:Config.MoviesLibraryPath -and (Test-Path $script:Config.MoviesLibraryPath)) {
                    Write-Host $currentMovies -ForegroundColor Green
                } elseif ($script:Config.MoviesLibraryPath) {
                    Write-Host "$currentMovies (not reachable)" -ForegroundColor Red
                } else {
                    Write-Host $currentMovies -ForegroundColor Yellow
                }

                # TV Shows Library
                $currentTV = if ($script:Config.TVShowsLibraryPath) { $script:Config.TVShowsLibraryPath } else { "(not set)" }
                Write-Host "  TV library: " -NoNewline -ForegroundColor White
                if ($script:Config.TVShowsLibraryPath -and (Test-Path $script:Config.TVShowsLibraryPath)) {
                    Write-Host $currentTV -ForegroundColor Green
                } elseif ($script:Config.TVShowsLibraryPath) {
                    Write-Host "$currentTV (not reachable)" -ForegroundColor Red
                } else {
                ...
                # Inbox
                $newInbox = Read-Host "Inbox path [$currentInbox]"
                if ($newInbox -eq 'browse') {
                    $newInbox = Select-FolderDialog -Description "Select your inbox folder" -InitialPath $script:Config.InboxPath
                }
                if ($newInbox) {
                    $script:Config.InboxPath = $newInbox
                    Write-Host "  Inbox set to: $newInbox" -ForegroundColor Green
                }

                # Movies Library
                $newMovies = Read-Host "Movies library path [$currentMovies]"
                if ($newMovies -eq 'browse') {
                    $newMovies = Select-FolderDialog -Description "Select your movies library folder" -InitialPath $script:Config.MoviesLibraryPath
                }
                if ($newMovies) {
                    $script:Config.MoviesLibraryPath = $newMovies
                    Write-Host "  Movies library set to: $newMovies" -ForegroundColor Green
                }

                # TV Shows Library
                $newTV = Read-Host "TV shows library path [$currentTV]"
                if ($newTV -eq 'browse') {
                    $newTV = Select-FolderDialog -Description "Select your TV shows library folder" -InitialPath $script:Config.TVShowsLibraryPath
                }
                if ($newTV) {
                    $script:Config.TVShowsLibraryPath = $newTV
