...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Version-5.8.2-orange.svg" alt="Version">
</p>
...
- **Stuck downloads resolved from the dashboard** - Radarr parks three kinds of grab and waits for a human: a multi-part RAR set ("Found archive file"), a release whose file names carry no year or quality ("Unable to parse file"), and a release that matched more than one library movie ("found multiple movies" — hidden from Radarr's own queue listing unless asked for). The `S` workflow and the Radarr Status utility walk each one: unrar it on the seedbox and hand the result to Radarr, or drive Radarr's own manual import with the quality read from the grab title (or the file name), asking which movie when Radarr matched several — always a hardlink, so the torrent keeps seeding. If the archive's video already sits in your inbox or library, it says so instead of making a second copy
