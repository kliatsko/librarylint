...
A pass over every place the pipeline could see a problem but not finish it.
The health check became a walk you can press Enter through, gained a check
that catches an NFO describing a different film entirely, and stopped
re-reporting findings it had no fix for. Radarr's stuck downloads are now
resolved from the dashboard instead of the web UI. The mirror reports what it
is actually doing and gives up in seconds when the destination disappears,
rather than in twenty minutes of red. Test coverage went from ad-hoc scripts
to 398 offline tests.

### Added
- **NFO identity check, by runtime.** "Split (2016)" passed every name-based check while holding an NFO for Deborah Kampmeier's 150-minute film and a video of Shyamalan's 117-minute one: same title, same year, well-formed NFO. Nothing that reads names can see that; the video's real length can. The health check now compares each NFO's runtime against the measured duration and flags a gap over 15% and at least five minutes, which clears the normal noise (TMDB runtimes are theatrical and rounded, rips lose credits, PAL speed-up takes four minutes off a hundred). Durations are cached in `release-info.json` keyed by file size, with at most 150 new probes per run, so a large library measures itself over a few runs instead of stalling one. The fix step proposes the same-title TMDB candidate whose runtime matches and regenerates the NFO from it; a long cut you confirm is stamped `IdentityConfirmed` and never flagged again. Placed before the folder-name step, because re-identifying an NFO can change its year.
- **Stuck Radarr downloads resolved from the dashboard.** Three kinds of grab sit in the queue waiting for a human, and each is one action away. A multi-part RAR set Radarr will not unpack is extracted on the seedbox with `unrar` and handed back as a `DownloadedMoviesScan` tied to the download id. A release whose file names carry no year or quality ("Unable to parse file" for `HELLBOY.mkv`) is driven through Radarr's own `ManualImport` with the movie taken from its by-download-id view and the quality parsed from the grab's release title. An ambiguous grab ("found multiple movies") asks which film it is and imports into that one. The walk is Y/N/Q like the health check, previews before acting, and an archive whose unpacked bytes already sit in the library is reported rather than extracted a second time. Items with no movie attached are invisible to Radarr's queue API unless `includeUnknownMovieItems` is set, so the most stuck item was the one the dashboard could not see; it now asks for those.
- **Leftover subtitle folders are found and removed.** A release's `Subs` folder is spent once its subtitles have been moved up beside the video, but what stays behind is a checksum or a tracker readme, and nothing removed it: subtitle processing moves subtitles only, Clean Unnecessary Files matches names, Remove Empty Folders wants nothing inside. Seven such folders sat in a 1,496-movie library. A folder now counts as spent when its name is a subtitle-folder name and no file under it at any depth is a subtitle or video. Initial processing sweeps them after its moves, the health check reports them as their own category listing the litter, and the placement repair removes the folder it just emptied. Every deletion re-tests the folder at that moment, so a subtitle that landed after the scan saves it.
- **Configurable resolution floor.** Nothing automatic ever looked at resolution: the sub-HD quality concern only fires below 720p and only on demand, so a library full of 720p rips never surfaced. New `FlagResolutionAtOrBelow` config key, default 0 (disabled) since what counts as too low is a personal call. Each movie resolves from the cheapest trustworthy source: a cached probe, else the release name recorded at import, else a budgeted MediaInfo probe. Status lists the count and the five worst, each labelled verified or taken from the release name. `Get-ResolutionTier` is now the single source of truth for the ladder and buckets by the larger dimension, so a 2.39:1 scope release at 1280x528 is 720p rather than 528p.
- **Hardsub findings act instead of reporting.** A confirmed hardsubbed rip goes straight to Radarr for re-acquisition from the audit and from the `S` workflow, through the same engine the utility menu uses; the CSV export is demoted to an opt-in "save a copy for your records". A release that admits the defect in its own filename (`HC`, `KORSUB`, `HARDSUB`) records itself as reviewed at classification time, because that is categorically different evidence from an OCR verdict. A new `ReacquisitionRequestedAt` stamp buckets confirmed hardsubs into not-yet-sent, awaiting, and stalled 30-plus days, so a request that goes nowhere is called out rather than falling silent.
- **A permanent test suite.** 398 offline tests across 18 files under `Tests/`, replacing this cycle's throwaway fixture scripts. Every regression test was mutation-checked: the fix reverted and the test confirmed to fail, which caught more than one test passing for the wrong reason.

### Changed
- **The health check is one ordered list, walked step by step.** The report and the fix menu were two hand-maintained lists in different orders, so the screen showed orphaned subtitles second while the menu offered them last. Both now come from a single definition, so they cannot drift. The order is the real dependency chain: files are renamed to match their folder and subtitles to match their video, so folder names must settle first or each step re-breaks the last. Pick-and-choose became a sequential Y/N/Q walk defaulting to yes, which makes Enter through the list what "fix all of the above" used to be. A re-scan is offered after any fix, since earlier fixes invalidate later counts.
- **Mirror progress comes from robocopy's own counters.** A mirror sat for an hour on "1% 4.75 GB (19/855) | 175 MB/s | Arsenic and Old Lace (1944)-trailer.mp4" while robocopy was about 100 GB in at 36 MB/s. Every number came from a source that cannot tell the truth, each verified experimentally: redirected stdout is block-buffered, so files finishing in the first second were announced seven seconds later; robocopy pre-allocates each destination file to full size the moment a thread starts it, so polling dest size reads 100% at once; NIC statistics count the same bytes once per NDIS filter driver stacked on the adapter, which turned 36 MB/s into 175; and a file line is written when a thread *starts* the file, so the name on screen was never the one being copied. Progress now tails robocopy's own `/UNILOG` live, takes bytes and speed from the process's `WriteTransferCount`, and labels the file "latest" rather than "current".
- **The mirror aborts when the destination goes away.** A share that vanished mid-copy was handled one file at a time, turning 1,413 pending files into twenty minutes of red while the stall watchdog's TCP probe kept succeeding against a host that was still up. Robocopy's own error codes now carry the decision: a file becomes a strike only after robocopy exhausts its retries on it, so a two-second SMB blip under `/MT:16` never aborts, while three distinct strikes or one destination-directory failure kills the run and prints what happened and whether the host or only the share is gone.
- **Sync downloads anything not already downloaded.** The daily sync refused any remote file whose parent folder matched a library folder, on the theory that a folder of that name meant the title was owned. That is precisely the state every deliberate upgrade starts from. Seventeen hardsubbed rips were sent to Radarr, Radarr found clean releases and imported them, and the sync turned fifteen away as "already in library", writing each into the tracking file as downloaded so the upgrade was never offered again. The folder tier is gone from the sync; whether a new copy beats the old one is decided once, at library-add time, by the existing quality comparison. The explicit reconcile tool keeps it, since "trust what is on disk" is what that tool is for.
- **A folder year is wrong only if the film had no cinema release in it.** Fix Folder Names proposed eleven year corrections that were mostly a festival-premiere year against the NFO's theatrical year: Enemy (Toronto 2013, cinemas 2014), 300, Casablanca, Cube, Frances Ha. `Get-TMDBMovieDetails` now returns every cinema release year (TMDB types 1 to 3), and a folder year is kept when it is one of them and sits within a year of the NFO's date. Digital and disc dates are excluded, so a folder named for the Blu-ray year is still flagged, and a re-release decades later cannot justify a wrong name. Eight of the eleven kept, Glengarry Glen Ross, Groundhog Day and The Town still corrected.
- **Fix Folder Names catches a year that disagrees with the NFO.** Its skip looked only at the shape of the name, so a well-formed "(Year)" was taken as a correct one and "The Town (2009)" sat in the library for months with an NFO, a TMDB id and an original filename all saying 2010.
- **One request path for Radarr and Sonarr.** `Invoke-ArrRequest` is now the single path for every call. The duplicated part was the plumbing, not the logic, so retry, timeout and connection handling are shared while the domain logic stays separate.
- **The hardsub audit's progress line counts library position**, not samples taken, and says "already audited" rather than "cached". A walk 62% through the library used to display `[96/1466]` and read as barely started.

### Fixed
- **A scan that found nothing counted as one finding.** "Empty Folders — 1", then "Cannot bind argument to parameter 'LiteralPath' because it is null", on a library with no empty folders. A pipeline that yields nothing leaves its variable `$null`, and `@($null)` is a one-element array, so the report printed a phantom and the walk offered to delete it. Empty folders, zero-byte files and small videos all had the shape.
- **A film's own parentheses were reported as a naming issue forever.** "Birdman or (The Unexpected Virtue of Ignorance) (2014)" and "Birds of Prey (and the Fantabulous Emancipation of One Harley Quinn) (2020)" failed the canonical-name rule on every run with no way to satisfy it, and the malformed-name gate then skipped both folders in the file-name and trailer checks for good. Parentheses inside a title are now accepted when balanced, not nested, made of words, and holding neither a year nor a release token, so "(1080p (2001)" and "(BluRay x264)" are still rejected.
- **The orphan-subtitle check and its repair disagreed permanently.** The check stripped eight language codes before comparing a subtitle with its video; the repair knew forty. A Danish `.da.idx` was therefore flagged every run while the repair offered to fix it found nothing to do. Both now split names through one rule, and a subtitle named exactly like its video is never an orphan whatever its last segment looks like.
- **Two fix steps asked to apply an empty plan.** When the folder-rename or subtitle dry run proposed nothing, the walk still asked "Apply these renames? [Y]". Each step now explains why nothing was offered, naming the folder or file and the reason (no trusted NFO, several videos in the folder, a name only the check objects to), and skips the prompt.
- **Fix File Names stripped subtitle qualifiers.** Its double-name cleanup knew one 2-to-3-letter language code and treated anything else between basename and extension as duplicated-title junk, so `Title (Year).en.forced.srt` became `Title (Year).srt`. It now peels every recognised qualifier back onto the suffix and only strips a remainder carrying a "(Year)".
- **Subtitle verification survived a merge that replaced the video.** When a higher-scoring copy won, the target kept its `.subs_ok` still claiming the subtitles were verified on the strength of a file that had just been deleted, and its `.srt` survived named for that deleted video, where the orphan repair would later rename it onto the incoming one. Both are now cleared, matched by basename per deleted video so a subtitle for a video being kept is untouched.
- **Sonarr had no retry and no reachability check.** Its three API calls were bare `Invoke-RestMethod`, so a transient SQLite lock aborted the whole run, and an unreachable instance surfaced as a confusing mid-flow failure after minutes of quality-scoring files. Also fixed in the shared path: the transient-error pattern matched "timeout" but not "timed out", which is how .NET phrases a socket timeout, and `-SuppressErrors` suppressed nothing because the catch rethrew regardless.
- **Cancelling a mirror could stamp partial files as complete.** The cancel path repaired timestamps on any announced file whose destination size matched the source, and with robocopy's pre-allocation every in-flight partial matches. Those files then looked identical to `/MIR` forever: a video that plays for a while and stops, with nothing in the pipeline able to notice. The repair is removed; completed files were never the problem, since robocopy stamps each as it finishes. An audit of the destination found three such files and re-stamped them for recopy.
- **The mirror counted extras as files to copy and never as files to delete.** With `/NC` robocopy drops the `*EXTRA File` label, so extras printed in the same shape as copies, and the summary's extras count lives in column 6 of the `Files` row rather than in an "Extras :" row that does not exist.
- **The extracted-sync re-downloaded eight movies the library had held for months.** Their `.extracted/` folders were leftovers of extractions Radarr imported long ago, and the tool only asked whether the release was in the inbox. Both it and the upgrade-tag bypass now compare exact byte size against a library index.

## [5.8.1] - 2026-09-08

Hardening pass over the hardsub audit that 5.8.0 introduced: it now sees burns
it was blind to, proves its verdicts visually, and remembers them so the audit
stops being a thing you re-run from scratch.

### Added
- **Hardsub verdicts are cached per movie** in each folder's `release-info.json` under a `HardsubAudit` key (verdict, coverage, samples, OCR languages, audit date, video fingerprint, human review). The fingerprint is size plus a hash of the first and last 64KB — content-based on purpose, since mirror timestamp repair, copies and renames all churn mtimes without changing the pixels a verdict depends on. A verdict is reused when the fingerprint matches and the prior run was at least as strong (same-or-more samples, same-or-wider language set); a stronger run re-audits, a replaced file re-audits, and `-Force` re-audits everything. New `Get-VideoFingerprint`, `Get-HardsubAuditRecord`, `Set-HardsubAuditRecord`, `Get-HardsubAuditBacklog`.
- **Human review is recorded and final** (`Set-HardsubReview`). After the report, the audit asks which flagged movies are real hardsubs and offers to file the rest as reviewed-clean. Reviewed movies are never re-flagged until their video file changes, so cleared false positives stay cleared instead of returning on every run. Reviewed hardsubs appear in a "known hardsubs" section rather than as fresh findings.
- **Release-name pre-pass.** Releases whose name carries an `HC`, `KORSUB`, `HARDSUB` or `HARDCODED` tag are classified `TAGGED` instantly, from the video filename or any release name in `release-info.json` — no frames sampled, no OCR, and it catches burns OCR cannot see at all when the matching language pack is absent. Surfaced 16 hardsubbed rips in a 1,463-movie library that frame sampling had missed.
- **Non-Latin OCR language packs, self-installing.** The audit now uses its own tessdata directory at `%LOCALAPPDATA%\LibraryLint\tessdata`, seeded from the system Tesseract install, so packs install without elevation (the winget install ships English only and lives under Program Files). Missing packs are offered for download from the official tessdata repository on first audit: Korean, Japanese, Chinese (simplified/traditional), Russian, Thai, Vietnamese, Arabic. KORSUB-style burns OCR as nothing under English-only data, which made the audit's canonical target invisible.
- **Visual HTML evidence report.** Flagged movies keep up to four frame pairs — the frame where text was detected beside the same shot ~2.5s later — embedded as JPEGs in one self-contained file under `Reports\`. Verifying a verdict becomes scrolling a page instead of scrubbing a player, and the pair makes subtitle-versus-scene-text obvious at a glance. Offered for opening when the audit finishes.
- **The `S` workflow drains the audit backlog.** New `HardsubChecksPerRun` config key (default 10, `0` disables): every Status run audits that many not-yet-audited movies, reports flagged titles inline, and shows how many remain. Tagged releases are free and don't consume the budget. Once the library is baselined this only ever touches new arrivals and replaced files.
- **Queue items are named on the Status dashboard.** Radarr and Sonarr queue rows now show the title (series plus `SxxEyy`, or movie plus year), download state (`downloading 63%`, `import pending`, `import blocked`, `failed`), severity colour, and the app's first status message — so a stuck import explains itself instead of showing as a bare count. Up to five in Status, all fetched items in the Radarr/Sonarr Status utility.
- **`hardsub` subtitle provider.** Confirmed hardsubs can optionally be recorded in `.subs_ok` as `Provider: hardsub`, which drops them from the subtitle census's missing bucket and out of the acquisition and Whisper pools. Opt-in and default no, since replacing the rip is usually the real fix.
- **Leftover show-shell cleanup.** A metadata-only show folder in the inbox whose series already exists in the TV library is reported as a removable leftover rather than waiting forever for episodes, with a merge-then-delete prompt that only copies files the library lacks. Container children under `_Shows` get the same shell check.

### Changed
- **Default frame sampling doubled to 48** per movie, with a per-run prompt to change it. Cost is roughly 25 seconds for a clean movie thanks to the adaptive language stack below.
- **Adaptive OCR language stack.** Tesseract reloads every model on each invocation, making the full nine-script stack about 17x the cost of an English-only call. Six probe frames spread across the runtime run the full stack; any text on a probe escalates the whole movie, while a movie whose probes read nothing finishes English-only. A burned film has text on most dialogue frames, so six probes missing it entirely is a sub-1% event.
- **The re-acquisition CSV is written after the review, not before**, and contains only confirmed hardsubs (this run's plus earlier confirmations still in the library). Exporting raw flags would queue false positives for re-download. Skipping the review leaves the unreviewed export available but defaulting to no.
- **Audit progress counter** now reads sampled-versus-cap with a running count of cached skips, instead of counting against the whole library — an uncapped counter made a mostly-cached run look like a full re-audit.
- **Failed audits are recorded** as a `FAILED` verdict (unreadable duration, no extractable frames) so they aren't retried every run; without this, one bad file at the top of the alphabet would consume a slot of the per-run budget forever. `-Force` retries them.
- **README accuracy pass.** The subtitle section documented the demoted Subdl-first flow as the whole feature; it now describes the tiered pipeline. Main menu table updated to the `S`-first layout, missing features added (Whisper, hardsub audit, HTPC control, Sonarr), dependency table gained Tesseract, ffsubsync and faster-whisper, API-key table gained OpenSubtitles. `GETTING_STARTED.md` pointed at a launcher filename that does not exist.

### Fixed
- **Int32 overflow made the verdict cache never hit.** `[Math]::Min(65536, $size)` binds the Int32 overload from the integer literal, so fingerprinting any file over 2.1GB threw inside its try/catch and returned null. Small files cached correctly while every full-size movie re-audited on each run — the failure was invisible because the exception was swallowed and the records looked fine apart from a null fingerprint. Clamp written manually. Existing records were repaired in place, so no OCR work was lost. Third occurrence of this footgun in this codebase.
- **A stray `!` line for healthy Radarr/Sonarr.** An empty health array arrives from the API as null, and wrapping null in `@()` yields a one-element array, so a perfectly healthy app rendered one empty warning row.
- **OCR mojibake.** Tesseract emits UTF-8, but PowerShell decodes native stdout using the console's OEM code page, turning each CJK or Hangul character into three Latin-range glyphs — which then passed the "multi-word Latin line" test as if it were real text. TSV output is now written to a file and read as UTF-8, sidestepping console encoding entirely.
- **Grain hallucinations counted as subtitles.** Non-Latin models invent glyphs from film grain, and because each frame's noise differs, the temporal check read them as "changing text" — the exact signature of a subtitle. Words now only count when a single OCR text line holds at least two confident words and one script owns at least 70% of that line's letters, which is structurally what a subtitle looks like and what scattered hallucinations are not. New `Get-TextScript` and `Select-SubtitleShapedWords`.
- **A custom tessdata directory broke OCR silently.** Passing `tsv` on the tesseract command line names a config file that lives under `tessdata/configs/`, which LibraryLint's own data directory does not have; the run emitted `read_params_file: Can't open tsv` and produced no output. The equivalent parameter is now passed inline.
- **`[Console]::WindowWidth` aborted a movie's whole audit iteration** in redirected consoles. Both uses are guarded.
